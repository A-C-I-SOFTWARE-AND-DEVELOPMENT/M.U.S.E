...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):  # ty: ignore[invalid-parameter-default]  # duck-typed platform/adapter path
        ...
        # muse_state._WAL_INCOMPAT_MARKERS).
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            try:
                from gateway.api.skills_api import create_skills_routes
                for route in create_skills_routes():
                    self._app.router.add_route(route.method, route.path, route.handler)
            except ImportError:
                pass  # aiohttp not available or skills_api not present
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from muse_cli.auth import has_usable_secret
