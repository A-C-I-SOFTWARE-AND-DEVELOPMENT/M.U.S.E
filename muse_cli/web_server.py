...
Usage:
    python -m muse_cli.main web          # Start on http://127.0.0.1:9119
    python -m muse_cli.main web --port 8080
"""
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``muse_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "muse_cli.main", *subcommand]
...
# in muse_cli/config.py — listed here for deterministic ordering in the UI.
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from muse_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from muse_cli.auth import clear_provider_auth
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from muse_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from muse_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from muse_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from muse_cli import profiles as profiles_mod
    try:
...
async def get_skills():
    ...
    from muse_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from muse_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from muse_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    matching what ``muse_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from muse_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as muse_cli.plugins):
    ...
    from muse_cli.plugins import get_bundled_plugins_dir
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from muse_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from muse_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from muse_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from muse_cli.plugins_cmd import dashboard_remove_user_plugin
