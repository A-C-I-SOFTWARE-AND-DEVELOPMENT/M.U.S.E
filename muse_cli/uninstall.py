...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains muse_cli reference,
                # or a legacy pre-rename hermes_cli / hermes-agent one)
                content = wrapper.read_text()
                if (
                    'muse_cli' in content
                    or 'muse-agent' in content
...
def uninstall_gateway_service():
    ...
    try:
        from muse_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from muse_cli.gateway import get_launchd_plist_path
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m muse_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "muse_cli.main", "--profile", name]
...
def run_uninstall(args):
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/A-C-I-SOFTWARE-AND-DEVELOPMENT/M.U.S.E/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/A-C-I-SOFTWARE-AND-DEVELOPMENT/M.U.S.E/main/scripts/install.sh | bash", Colors.DIM))
        print()
