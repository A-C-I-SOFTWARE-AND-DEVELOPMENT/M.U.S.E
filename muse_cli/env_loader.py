...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``muse_cli.config._sanitize_env_lines`` which
    ...
    if not path.exists():
        return
    try:
        from muse_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    if hermes_home:
        ...
        # Full resolution (override -> MUSE_HOME -> HERMES_HOME -> existing
        # ~/.muse | ~/.hermes -> ~/.muse) so fresh post-rename installs read
        # ~/.muse/.env instead of a hardcoded ~/.hermes.
