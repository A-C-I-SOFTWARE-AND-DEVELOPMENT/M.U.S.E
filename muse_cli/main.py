...
# IMPORTANT: muse_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``muse_bootstrap`` is a
...
# new code references ``muse_bootstrap`` but the editable install's
...
def _apply_profile_override() -> None:
    ...
    # Mirrors muse_cli.profiles._PROFILE_ID_RE so we never call
...
# var BEFORE muse_logging imports agent.redact (which snapshots the flag at
...
try:
    from muse_logging import setup_logging as _setup_logging
...
try:
    from muse_cli.config import load_config as _load_config_early
    from muse_constants import apply_ipv4_preference as _apply_ipv4
...
from muse_cli import __version__, __release_date__
from muse_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from muse_cli.config import get_env_path, get_hermes_home, load_config
    from muse_cli.auth import get_auth_status
    ...
    from muse_cli.auth import PROVIDER_REGISTRY
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from muse_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from muse_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    if code == 42:
        from muse_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from muse_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        from muse_cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from muse_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from muse_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from muse_cli.config import get_env_value, save_env_value
...
def cmd_setup(args):
    ...
    from muse_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from muse_cli.config import stamp_install_method
    from muse_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from muse_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from muse_cli.config import read_raw_config
    ...
    from muse_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _clear_stale_openai_base_url():
    ...
    """
    from muse_cli.config import get_env_value, save_env_value, load_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from muse_cli.config import load_config
    from muse_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from muse_cli.auth import _prompt_model_selection
    from muse_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from muse_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from muse_cli.config import get_env_value
    ...
    from muse_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from muse_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from muse_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from muse_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from muse_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from muse_cli.auth import resolve_codex_runtime_credentials
    ...
    if selected:
        ...
        _save_model_choice(selected)
        try:
            from muse_cli.config import save_config, load_config
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from muse_cli.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from muse_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from muse_cli.auth import _save_model_choice, deactivate_provider
    from muse_cli.config import get_env_value, load_config, save_config
    ...
    from muse_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from muse_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from muse_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from muse_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from muse_cli import azure_detect
    import getpass
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from muse_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from muse_cli.config import save_env_value, load_config, save_config
    from muse_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from muse_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from muse_cli.copilot_auth import validate_copilot_token
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    from muse_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from muse_cli.config import save_env_value
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from muse_cli.auth import AuthError
        from muse_cli.models import fetch_lmstudio_models
        ...
        from muse_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from muse_cli.auth import get_anthropic_key
...
def cmd_login(args):
    ...
    from muse_cli.auth_commands import headless_login_command
...
def cmd_logout(args):
    ...
    from muse_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from muse_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from muse_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from muse_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from muse_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from muse_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from muse_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from muse_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from muse_cli.doctor import run_doctor
...
def cmd_jarvis(args):
    ...
    if action == "launch":
        from muse_cli.jarvis_prime.launch import launch as _launch
...
def cmd_models(args):
    ...
    if action == "bootstrap":
        from muse_cli.jarvis_prime.model_bootstrap import bootstrap as _bootstrap
...
def cmd_dump(args):
    ...
    from muse_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from muse_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from muse_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from muse_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from muse_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from muse_cli.backup import run_import
...
def cmd_version(args):
    ...
    try:
        from muse_cli.banner import check_for_updates
        from muse_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from muse_cli.uninstall import run_uninstall
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``muse_cli/web_dist/`` (per vite.config.ts
    outDir: "../muse_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "muse_cli" / "web_dist"
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m muse_cli.main web` invocation.
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    patterns = [
        "hermes dashboard",
        "muse dashboard",
        "muse_cli.main dashboard",
        "muse_cli/main.py dashboard",
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `muse_cli.gateway._scan_gateway_pids` and avoids the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # muse_cli.config.get_hermes_home to simulate setup failure.
        from muse_cli.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check():
    ...
    if method == "pip":
        from muse_cli.config import recommended_update_command
        from muse_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from muse_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        from muse_constants import get_hermes_home, display_hermes_home
...
def cmd_update(args):
    ...
    """
    from muse_cli.config import is_managed, managed_error
...
def _cmd_update_pip(args):
    ...
    from muse_cli import __version__
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # See muse_cli/branch_consolidation.py. Non-fork installs and
        ...
        try:
            from muse_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in muse_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_hermes_home added to muse_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload muse_constants so that any lazy
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from muse_cli.tools_config import install_cua_driver
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from muse_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from muse_constants import get_hermes_home as _hh
        ...
        from muse_cli import profile_describer as _pd
        ...
        from muse_cli.profiles import profile_exists
        ...
        from muse_cli.profiles import rename_profile
        ...
        from muse_cli.profiles import export_profile
        ...
        from muse_cli.profiles import import_profile
        ...
        from muse_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from muse_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from muse_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "muse_cli" / "web_dist"
        )
    ...
    from muse_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from muse_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from muse_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from muse_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``muse_cli/_parser.py``.
#
...
def main():
    ...
    try:
        from muse_cli.stdio import configure_windows_stdio
    ...
    from muse_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from muse_cli.fallback_cmd import cmd_fallback
    ...
    # =========================================================================
    from muse_cli.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from muse_cli.kanban import build_parser as _build_kanban_parser
    ...
    # ``muse_cli.guardrails_cli`` (NOT under ``jarvis_prime``) on purpose:
    ...
    # =========================================================================
    from muse_cli.guardrails_cli import register as _register_guardrails
    ...
    # promote. Logic lives in muse_cli/jarvis_prime/gemma_cli.py — this is just
...
""",
    ...
    from muse_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    def cmd_pairing(args):
        from muse_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from muse_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from muse_cli.skills_hub import skills_command
    ...
    from muse_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    def cmd_plugins(args):
        from muse_cli.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from muse_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from muse_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from muse_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from muse_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from muse_cli.tools_config import tools_command
    ...
    def cmd_claw(args):
        from muse_cli.claw import claw_command
...
""",
    ...
    from muse_cli.config import get_container_exec_info
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from muse_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from muse_cli.oneshot import run_oneshot
