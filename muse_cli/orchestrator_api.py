...
class Job:
    ...
    def budget_status(self) -> Optional[Dict[str, Any]]:
        ...
        :class:`~muse_cli.budget_policy.BudgetDecision`.
        """
...
def _budget_status_for(
    ...
    otherwise the :class:`~muse_cli.budget_policy.BudgetDecision` rendered as
...
class _ReportedUsage:
    ...
    ``UsageLike`` protocol that :meth:`muse_cli.job_cost.JobCost.add_usage`
...
def _extract_usage_report(
    ...
    This is the producer seam documented in ``muse_cli.job_cost``: a worker
...
class JobStore:
    ...
    delegated to :class:`muse_cli.orchestrator_events.EventBroker`;
    ...
    async def accumulate_cost(
        ...
        *,
        ...
        :meth:`muse_cli.job_cost.JobCost.add_usage`. ``usage`` is any object
        ...
        — see the module docstring of ``muse_cli.job_cost``.
        """
        async with self._lock:
            job = self._jobs.get(job_id)
            if job is None:
    ...
    async def snapshot(self, job_id: str) -> JobSnapshot:
        ...
        (:func:`muse_cli.job_replay.rebuild_snapshot`). This is the basis
    ...
    def restore_from_disk(self) -> int:
        ...
        For every job id that :func:`muse_cli.job_event_store.iter_job_ids`
        reports, this reads the persisted envelopes, folds them through
        :func:`muse_cli.job_replay.rebuild_snapshot`, and reconstructs a
        ...
        Known limitation: :class:`muse_cli.job_cost.JobCost` is **not**
...
def create_app(
    *,
    ...
    if not FASTAPI_AVAILABLE:
        raise RuntimeError(
            "fastapi is required for muse_cli.orchestrator_api. Install "
    ...
    async def get_job_snapshot(job_id: str) -> Dict[str, Any]:
        ...
        (:func:`muse_cli.job_replay.rebuild_snapshot`) to rebuild status /
        ...
        (:mod:`muse_cli.job_event_store`), and :func:`create_app` calls
    ...
    async def update_worker(
        ...
        in ``muse_cli.job_cost``: a worker that knows its per-call model
