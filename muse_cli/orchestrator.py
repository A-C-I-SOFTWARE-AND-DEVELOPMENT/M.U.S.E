...
# (see :mod:`muse_cli.orchestrator_ledger` and ``docs/orchestration/README.md``).
# That's the path :mod:`muse_cli.jarvis_prime.awareness` reads when it
...
# (``muse_cli.budget_policy.evaluate_budget``). The single-job ``dispatch_job``
...
def _budget_stop_for_spend(
    ...
    *,
    ...
    if soft_limit is None and hard_limit is None:
        return None
    try:
        from muse_cli.budget_policy import evaluate_budget
...
def navigate_job(
    ...
    *,
    ...
    HyperAgent-style :class:`~muse_cli.jarvis_prime.navigation.Navigator`,
...
def dispatch_job(
    ...
    *,
    ...
    (:func:`muse_cli.budget_policy.evaluate_budget`). Both limits default to
    ...
    from muse_cli.workers import registry as _wr
...
def validate_job(job_id: str) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        # be missing ``muse_cli.validation``.  Surface a skeleton record
...
def run_swarm(rest: str) -> str:
    ...
    if not rest or rest in {"-h", "--help", "?", "help"}:
        return _SWARM_HELP
    try:
        from muse_cli.swarm.coordinator import run_swarm as _run_swarm
