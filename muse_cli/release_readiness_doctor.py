...
Mirrors :mod:`muse_cli.jarvis_prime.launch_doctor`.
"""
...
def _repo_root() -> Path:
    # muse_cli/release_readiness_doctor.py -> repo root is one level up.
...
def _check_package_import() -> ReadinessCheck:
    ok = _importable("muse_cli") and _importable("run_agent")
    return ReadinessCheck(
        "package import",
        PASS if ok else FAIL,
        "muse_cli + run_agent import" if ok else "core package does not import",
    )


def _check_decision_engine_present() -> ReadinessCheck:
    ok = _importable(
        "muse_cli.decision_engine", "merge_decision_inputs", "DecisionVerdict"
    )
    return ReadinessCheck(
        ...
        if ok
        else "muse_cli.decision_engine missing the verdict API",
    )


def _check_owner_gate() -> ReadinessCheck:
    ok = _contains(
        "muse_cli/jarvis_prime/owner_auth.py",
...
def _check_redaction() -> ReadinessCheck:
    ok = _importable("muse_cli.secrets_policy", "redact") and _exists(
...
def _check_publisher_dry_run_default() -> ReadinessCheck:
    ...
    default = _param_default("muse_cli/github_publisher.py", "run", "approve")
    ok = default is False
    return ReadinessCheck(
...
def _check_bridge_signed_envelope() -> ReadinessCheck:
    ok = _contains("muse_cli/remote_bridge.py", "bridge_envelope") and _importable(
        "muse_cli.bridge_envelope"
    )
    return ReadinessCheck(
...
def _check_bridge_command_allowlist() -> ReadinessCheck:
    ok = _contains_any(
        "muse_cli/remote_bridge.py", "command_allowlist", "COMMAND_ALLOWLIST"
    )
    return ReadinessCheck(
...
def _check_verdict_at_publish() -> ReadinessCheck:
    ok = _contains("muse_cli/github_publisher.py", "merge_decision_inputs")
    return ReadinessCheck(
...
def _check_verdict_at_dispatch() -> ReadinessCheck:
    wired = any(
        _contains(f"muse_cli/{f}", "merge_decision_inputs")
...
def _check_budget_enforced() -> ReadinessCheck:
    ...
    single = _path_enforces("muse_cli/orchestrator.py")
    parallel = _path_enforces("muse_cli/orchestrator_parallel.py")
...
def _check_publisher_repo_allowlist() -> ReadinessCheck:
    ok = _contains_any(
        "muse_cli/github_publisher.py", "allowlist", "allowed_repo", "repo_allow"
    )
    return ReadinessCheck(
...
def _check_worker_leases_wired() -> ReadinessCheck:
    ok = _contains("muse_cli/orchestrator_parallel.py", "worker_lease")
    return ReadinessCheck(
