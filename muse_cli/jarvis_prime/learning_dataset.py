...
(:func:`muse_cli.jarvis_prime.gates.run_gate_summary`).

Clean-room, stdlib-only, local JSONL persistence with atomic writes and
``0o600`` perms — mirrors :class:`muse_cli.jarvis_prime.research_vault.ResearchVault`.
"""
...
class QualityGates:
    ...
    def from_gate_summary(
        ...
        Reuses :func:`muse_cli.jarvis_prime.gates.run_gate_summary` rather
...
class DatasetStore:
    ...
    def add_candidate(
        ...
        *,
        ...
        if not is_negative:
            from muse_cli.jarvis_prime.behavioral_risk import reward_hacking_evidence
