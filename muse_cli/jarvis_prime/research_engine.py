...
1. **Decompose** the query into sub-questions (reuses
   :mod:`muse_cli.jarvis_prime.research`'s ``ResearchBrief``).
...
3. **Rank** raw sources into :class:`~muse_cli.jarvis_prime.research_vault.EvidenceStrength`
   tiers via deterministic domain heuristics.
4. **Cards** — each ranked source is stored in the existing
   :class:`~muse_cli.jarvis_prime.research_vault.ResearchVault` (excerpt-only
...
8. **Final answer** with calibrated uncertainty via
   :func:`muse_cli.jarvis_prime.epistemics.audit_response`.
...
from muse_cli.jarvis_prime.epistemics import audit_response
