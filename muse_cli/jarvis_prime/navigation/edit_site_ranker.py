...
*edit-site packets* a coding worker (Claude Code / Codex / Aider / Goose /
local model) can act on without re-deriving context:
...
class EditSiteRanker:
    ...
    def rank(
        self, issue: str, *, limit: int = 5, max_dependents: int = 8
    ) -> list[EditSite]:
        localized = self.localizer.localize(issue, limit=limit)
