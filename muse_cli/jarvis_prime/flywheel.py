...
CLI:
    python -m muse_cli.jarvis_prime.flywheel digest [--hours H]
    python -m muse_cli.jarvis_prime.flywheel pending
"""
    ...
    try:
        ...
        nightly = create_job(
            ...
            script=_install_script(
                "flywheel-nightly.sh",
                "#!/usr/bin/env bash\n"
                "python -m muse_cli.jarvis_prime.flywheel digest\n"
                "python -m muse_cli.jarvis_prime.axiom_bridge audit\n",
            ),
            no_agent=True,
            deliver="local",
            workdir=workdir,
        ...
        weekly = create_job(
            ...
            script=_install_script(
                "flywheel-weekly.sh",
                "#!/usr/bin/env bash\n"
                "python -m muse_cli.jarvis_prime.flywheel file-pending --dir .plans\n",
            ),
            no_agent=True,
            deliver="local",
            workdir=workdir,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m muse_cli.jarvis_prime.flywheel",
