...
def _check_package_import() -> LaunchCheck:
    try:
        import muse_cli  # noqa: F401

        return LaunchCheck("package_import", PASS, "muse_cli imports")
    except Exception as exc:
        return LaunchCheck("package_import", FAIL, f"muse_cli import failed: {exc}")


def _check_cli_entrypoint() -> LaunchCheck:
    spec = importlib.util.find_spec("muse_cli.main")
    if spec is not None:
        return LaunchCheck(
            "cli_entrypoint",
            PASS,
            "muse_cli.main resolvable (console_script: hermes)",
        )
    return LaunchCheck("cli_entrypoint", FAIL, "muse_cli.main not found")


def _check_jarvis_import() -> LaunchCheck:
    try:
        import muse_cli.jarvis_prime  # noqa: F401

        return LaunchCheck(
            "jarvis_prime_import", PASS, "muse_cli.jarvis_prime imports (stdlib-only)"
        )
    except Exception as exc:
...
def _check_jarvis_module_runnable() -> LaunchCheck:
    spec = importlib.util.find_spec("muse_cli.jarvis_prime.__main__")
    if spec is not None:
        return LaunchCheck(
            "jarvis_module_runnable",
            PASS,
            "python -m muse_cli.jarvis_prime available",
        )
...
def _check_owner_gate() -> LaunchCheck:
    try:
        from muse_cli.jarvis_prime.owner_auth import AUTHORIZATION_PHRASE, OwnerAuth
...
def _check_termux_compat() -> LaunchCheck:
    ...
    modules = (
        ...
        "muse_cli.jarvis_prime.worker_locks",
        "muse_cli.jarvis_prime.worker_registry",
        "muse_cli.jarvis_prime.launch",
        "muse_cli.jarvis_prime.launch_doctor",
    )
...
def _check_secret_scan_operational() -> LaunchCheck:
    try:
        from muse_cli.jarvis_prime.guardrail_collectors import (
