...
:class:`~muse_cli.jarvis_prime.self_update.Proposal` that requires the
...
from muse_cli.jarvis_prime.gates import GateOutcome, GateResult
...
def _kind_for_target(target_path: str) -> ProposalKind:
    ...
    # hermes_cli is the legacy pre-rename path; stored proposals may
    # still reference it.
    if (
        "/muse_cli/" in p
        or p.startswith("muse_cli/")
        or "/hermes_cli/" in p
        or p.startswith("hermes_cli/")
    ):
