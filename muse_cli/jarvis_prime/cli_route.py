"""``route`` subcommand for ``python -m muse_cli.jarvis_prime``.
...
routing decisions come from :mod:`muse_cli.jarvis_prime.task_router`.
"""
...
def add_route_parser(subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    inline, so ``python -m muse_cli.jarvis_prime route ...`` behaves
    byte-identically.
    """
