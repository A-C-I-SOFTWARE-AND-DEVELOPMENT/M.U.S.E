...
(code first, so docs/evidence/ledger can attach CITES/DEPENDS_ON edges onto
existing FILE nodes). Every indexer is best-effort: one failing source never
aborts the build, mirroring ``orchestrator.navigate_job``'s non-blocking
contract. The result is an additive cache, safe to delete and rebuild.
"""
...
from muse_cli.jarvis_prime.graphrag.graph import KnowledgeGraph
