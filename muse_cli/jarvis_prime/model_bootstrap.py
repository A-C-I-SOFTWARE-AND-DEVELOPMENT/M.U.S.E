...
* **Local model layer (``muse_cli.local_models``).** Hardware probe,
...
def probe_hardware(hardware: Any = None) -> Any:
    ...
    if hardware is not None:
        return hardware
    try:
        from muse_cli.local_models import probe
...
def build_local_plan(
    *,
    ...
    try:
        from muse_cli.local_models import plan_bootstrap
...
def execute_local_downloads(
    ...
    *,
    ...
    if plan_obj is None:
        return []
    try:
        from muse_cli.local_models import execute_bootstrap
