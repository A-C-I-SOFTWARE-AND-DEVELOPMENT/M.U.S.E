...
class CompanionPresencePolicy:
    ...
    def state_for(self, signals: PresenceSignals) -> PresenceState:
        ...
        if signals.target_app:
            if signals.target_on_next_screen:
                return PresenceState.TURNING_PAGE
            return PresenceState.MOVING_TO_TARGET
        if signals.working:
        ...
        *,
        ...
        if target_app:
            if target_on_next_screen:
                steps.append(AnimationStep(PresenceState.TURNING_PAGE, f"JARVIS turns to the screen with {target_app}."))
            steps.append(AnimationStep(PresenceState.MOVING_TO_TARGET, f"JARVIS moves toward {target_app}."))
            steps.append(
                AnimationStep(
        ...
        return TaskAnimationPlan(
            ...
            execution_note=(
                ...
                "through Android permission education, accessibility checks, and owner gates."
            ),
        )
