"""CLI for ``python -m muse_cli.jarvis_prime``.
...
from muse_cli.jarvis_prime.awareness import perceive
from muse_cli.jarvis_prime.cli_route import add_route_parser
from muse_cli.jarvis_prime.gates import GATES, run_gate_summary
...
from muse_cli.jarvis_prime.tick import run_once as tick_once
...
def _cmd_registry_update(args: argparse.Namespace) -> int:
    ...
    try:
        from muse_cli.model_catalog import get_catalog
    ...
    print(
        ...
        f"(run `python -m muse_cli.jarvis_prime proposals list`). "
...
def _set_proposal_status(proposal_id: str, new_status: str, note: str) -> int:
    ...
    if not matched:
        print(
            ...
            f"(run `python -m muse_cli.jarvis_prime proposals list` to see ids)",
            file=sys.stderr,
        )
        return 1
...
def _cmd_learning_query(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.learning_analytics import query_dataset
...
def _cmd_memory_sources(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.memory_sources import REGISTRY, source_enabled
...
def _cmd_memory_ingest(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.memory_sources import ingest
...
def _cmd_learning_ingest_trajectory(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.learning_dataset import QualityGates
    from muse_cli.jarvis_prime.learning_ingest import from_trajectory_file
...
def _cmd_learning_free_recipe(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime.free_training import (
        TrainingStage,
        generate_recipe,
    )
...
def _cmd_learning_free_plan(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime.free_training import FreeContinuousPlan
...
def _cmd_learning_free_loop(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime.free_training import TrainingStage, run_free_loop

    if getattr(args, "stages", None):
        stages = tuple(TrainingStage(s) for s in args.stages)
    else:
        from muse_cli.jarvis_prime.free_training import DEFAULT_LOOP_STAGES
...
def _data_sources_pool(args: argparse.Namespace):
    ...
    from muse_cli.jarvis_prime.open_data_sources import load_registry
...
def _cmd_data_sources_list(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.open_data_sources import DatasetRole
...
def _cmd_data_sources_show(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.open_data_sources import get
...
def _cmd_data_sources_register_vault(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.open_data_sources import register_all_in_vault
...
def _cmd_avatar(args: argparse.Namespace) -> int:
    from muse_cli.jarvis_prime.avatar import default_avatar
...
def _cmd_compile(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime.ir_compilers.automation_flow import AutomationFlow
...
def _cmd_flow_exec(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime import nl_compile
    from muse_cli.jarvis_prime.ir_compilers.automation_flow import AutomationFlow
    from muse_cli.jarvis_prime.nlp_flow_exec import FlowExecutor
...
def _cmd_learning_prepare_job(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime.nlp_training import prepare_finetune_job
...
def _cmd_memory_tree(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime.memory_tree import MemoryTree
...
def _cmd_context(args: argparse.Namespace) -> int:
    ...
    """
    from muse_cli.jarvis_prime.context_handoff import build_context_handoff
...
def _cmd_self_audit_run(args: argparse.Namespace) -> int:
    ...
    import muse_cli.jarvis_prime.self_audit.seeds as sa_seeds
...
def _cmd_research_fabric(args: argparse.Namespace) -> int:
    ...
    from muse_cli.jarvis_prime.research_fabric.main import cli_main as _rf_cli
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m muse_cli.jarvis_prime",
