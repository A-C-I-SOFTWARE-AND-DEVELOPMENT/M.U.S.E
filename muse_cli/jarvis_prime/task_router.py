...
* **policy** — :mod:`muse_cli.jarvis_prime.model_bootstrap`
  (``load_policy``): free/local-first route order + paid explicit opt-in.
* **evidence** — :mod:`muse_cli.jarvis_prime.model_scorecard`
...
def _hosted_candidates(route: dict[str, Any], profile: TaskProfile) -> list[str]:
    ...
    *sunk below* the verified families. This is a stable partition — relative
    ...
    if not providers or not _hosted_taskclass_enabled():
        return providers
    try:
        from muse_cli.oss_model_brain import load_oss_catalog
...
def route_for_task(
    ...
    *,
    ...
    if measured:
        try:
            from muse_cli.jarvis_prime.model_scorecard import model_family
