...
_PROFILE_DIRS = [
    ...
    # See muse_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": str(profile_dir), "MUSE_HOME": str(profile_dir)},
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    old_muse_home = os.environ.get("MUSE_HOME")
    try:
        os.environ["HERMES_HOME"] = str(profile_dir)
        os.environ["MUSE_HOME"] = str(profile_dir)
        from muse_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_muse_home is not None:
            os.environ["MUSE_HOME"] = old_muse_home
        elif "MUSE_HOME" in os.environ:
            del os.environ["MUSE_HOME"]
