...
def run_job_gates(
    ...
    *,
    ...
    """
    from muse_cli.jarvis_prime.gates import gates_for_profile, run_gate_summary
...
class JobController:
    ...
    def _render_pr_body(job: Job, *, included: list[str]) -> str:
        ...
        lines.append("")
        lines.append(f"Hermes orchestrator job `{job.job_id}` (mode `{job.mode}`).")
        lines.append("")
        lines.append("## Prompt")
        lines.append("")
        lines.append("```")
        ...
        lines.append("")
        lines.append(
            "_Generated by the Hermes local orchestrator. Every handoff to an"
