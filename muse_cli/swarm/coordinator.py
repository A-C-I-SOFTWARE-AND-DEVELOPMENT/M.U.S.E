...
from muse_cli.swarm.grain import Grain, SwarmPlan, now_iso
from muse_cli.swarm import grainler as _grainler
from muse_cli.swarm import specialist as _specialist
from muse_cli.swarm.specialist import GrainAgentSpec
...
class PromptOnlyExecutor:
    ...
    def run(
        ...
        from muse_cli.orchestrator_parallel import ParallelRunner
...
def _converge(results: Sequence[SwarmGrainResult]) -> dict[str, Any]:
    ...
    try:
        from muse_cli.swarm.converge import converge_cooperative
    except Exception:
...
def run_swarm(
    ...
    *,
    ...
    :class:`~muse_cli.swarm.grain.OverlapError` (from partitioning) if the
    ...
    if decomposer is None and grains is None:
        from muse_cli.swarm.decompose import directory_decomposer
    ...
    try:
        ...
        try:
            from muse_cli.swarm.learning import capture_swarm_trace
        ...
        if apply_reversible:
            ...
            if effective_apply is None:
                try:
                    from muse_cli.swarm.learning import record_applied_update
