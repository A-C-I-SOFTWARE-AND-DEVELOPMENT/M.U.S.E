"""CLI entry: ``python -m muse_cli.swarm "<goal>" [--repo .] [--grains specs.json]``.

A thin, additive command surface over :func:`muse_cli.swarm.coordinator.run_swarm`.
...
from muse_cli.swarm.coordinator import run_swarm
from muse_cli.swarm.grain import OverlapError


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m muse_cli.swarm",
    ...
    decomposer = None
    if grains is None:
        from muse_cli.swarm import decompose as _d
        from muse_cli.swarm.grainler import default_decomposer
