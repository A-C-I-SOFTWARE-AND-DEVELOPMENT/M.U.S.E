...
- Conflict resolution reuses the stdlib model bridge
  (:mod:`muse_cli.jarvis_prime.self_audit.model_bridge`).  When no model
...
:mod:`muse_cli.main`.  The caller passes in a ``validate_syntax`` callback
...
def consolidate_into_main(
    ...
    *,
    ...
    (passed in to avoid importing :mod:`muse_cli.main`).  ``complete_fn`` /
    ...
    if complete_fn is None or is_model_configured is None:
        from muse_cli.jarvis_prime.self_audit import model_bridge
