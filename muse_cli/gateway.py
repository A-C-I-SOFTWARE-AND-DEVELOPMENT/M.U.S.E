...
# when muse_constants is cached from a pre-update version during `hermes update`.
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "muse_cli.main --profile",
        "muse_cli.main -p",
        "muse_cli/main.py gateway",
        "muse_cli/main.py --profile",
        "muse_cli/main.py -p",
        # Legacy pre-rename cmdlines: a gateway started before the
        # hermes_cli -> muse_cli rename must still be found/stoppable.
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "muse_cli.main"]
    if profile != "default":
...
def _gateway_list() -> None:
    ...
    """
    try:
        from muse_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from muse_constants import is_container, is_termux, is_wsl
...
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "muse_cli.main gateway",
    "muse_cli/main.py gateway",
    # Pre-rename (hermes_cli -> muse_cli) unit files must still be flagged.
    "hermes_cli.main gateway",
    "hermes_cli/main.py gateway",
    "gateway/run.py",
    " hermes gateway ",
...
User={username}
Group={group_name}
ExecStart={python_path} -m muse_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def _setup_whatsapp():
    ...
    from muse_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from muse_cli.dingtalk_auth import dingtalk_qr_auth
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``muse_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from muse_cli import setup as _s
    return {
