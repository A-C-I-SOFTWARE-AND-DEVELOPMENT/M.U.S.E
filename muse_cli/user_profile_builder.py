...
:mod:`muse_cli.github_history` for raw signal, rolls the signal into
    ...
    *,
    ...
    lines.extend([
        ...
        *_fmt_bullet_pair(
        ...
        "`muse_cli.user_profile_builder` with `--approve` only when "
...
def _detect_project_types(
    ...
    rules = [
        # hermes_cli kept: classifies historical commits from before the rename.
        ("Python tooling", re.compile(r"^(muse_cli|hermes_cli|tools|plugins|agent)/.*\.py$")),
        ("Android cockpit", re.compile(r"^apps/android/")),
        ("Gateway adapters", re.compile(r"^gateway/")),
        ("Orchestration", re.compile(r"^(docs/orchestration|muse_cli/orchestrator|hermes_cli/orchestrator)")),
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m muse_cli.user_profile_builder",
        description=(
