...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `muse skills browse` (labeled "official") and install it with `muse skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `muse skills install`.
...
- Integrate with `muse memory setup` via `post_setup()` — no need to touch core code
...
├── muse_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── muse_cli/               # CLI command implementations
...
- **Session persistence**: All conversations are stored in SQLite (`muse_state.py`) with full-text search and unique session titles. JSON logs go to `~/.hermes/sessions/`.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `muse setup` or update `~/.hermes/.env` locally.
...
- **Test it.** Run `muse --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Add to `_BUILTIN_SKINS` dict in `muse_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `muse_cli/skin_engine.py` for the full schema and existing skins as examples.
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `muse_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `muse_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `muse_cli/gateway_windows.py::
