/**
 * TerminalMode — embeds `hermes --tui` inside the dashboard's /chat tab.
 *
 ...
 *
 * Reachable as the "Terminal" mode of /chat (see pages/ChatPage.tsx). The
 * host keeps this component mounted once first activated and passes
 * `isActive` = tab-active && terminal-mode-selected, so the PTY child,
 * WebSocket, and xterm instance survive both tab switches and mode toggles.
 */
...
export function TerminalMode({ isActive = true }: { isActive?: boolean }) {
  ...
  // The dashboard keeps the /chat tab mounted persistently so the PTY
  // survives tab switches and mode toggles. That is great for ordinary
  // navigation, but it means query param changes do NOT remount the
  // component. Resume-in-chat from the Sessions page relies on
  // `/chat?resume=<id>` changing at runtime, so we must treat the current
  // resume target as part of the PTY identity and rebuild the terminal
  ...
  useEffect(() => {
    // When hidden (non-chat tab, or the conversational chat mode is
    // selected) we must not register the header button — another page owns
    // the header's end slot at that point.
    if (!isActive) {
      setEnd(null);
      return;
    }
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner muse TUI has enabled xterm mouse-events
    ...
    // forwarding them into muse. Keyboard input, paste, and resize still
    // behave normally.
  ...
  // When the terminal becomes visible again (isActive: false → true — tab
  // switch or chat→terminal mode toggle), the terminal host just
  // transitioned from display:none to display:flex. ResizeObserver won't
  // fire on that kind of style-driven box change — xterm thinks its grid
  // is still whatever it was when the tab was hidden (or 0×0, if it was
  // hidden before first fit).  Force a refit after two animation frames so
  // layout has committed.
  //
  // Focus handling: we only steal focus back into the terminal when
  // nothing else inside TerminalMode was holding it (typically the first
