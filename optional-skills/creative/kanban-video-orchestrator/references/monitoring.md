...
muse kanban watch --tenant <project-slug>

# Snapshot of the board
muse kanban list --tenant <project-slug>
muse kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
muse kanban stats --tenant <project-slug>

# Visual dashboard (browser)
muse dashboard

# Inspect a specific task (includes comments + events)
muse kanban show <task-id>

# Follow a single task's event stream
muse kanban tail <task-id>
```

Verify available subcommands with `muse kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `muse kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `muse kanban block <id>` then `muse kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `muse kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `muse kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `muse profile list` (profile exists?), `hermes status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(muse kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
muse kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
muse kanban block <task-id>
muse kanban archive <task-id>

# Diagnose what happened
muse kanban show <task-id>      # task body, comments, recent events
muse kanban tail <task-id>      # follow the live event stream
muse kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `muse kanban create` for the director
...
while true; do
    clear
    muse kanban list --tenant <slug>
    echo "---"
    muse kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `muse kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `muse profile list` and start
