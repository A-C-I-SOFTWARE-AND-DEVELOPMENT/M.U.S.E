...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept muse already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — muse provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different muse provider
        # (must be one muse already knows about — same names as
        ...
        # plugin sends — muse does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through muse' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing muse subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/A-C-I-SOFTWARE-AND-DEVELOPMENT/muse/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/A-C-I-SOFTWARE-AND-DEVELOPMENT/muse/blob/main/tests/agent/test_plugin_llm.py)
