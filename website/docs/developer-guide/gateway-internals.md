...
The messaging gateway is the long-running process that connects M.U.S.E. to 20+ external messaging platforms through a unified architecture.
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`muse send` CLI](/docs/guides/pipe-script-output) wrapping the same tool for shell scripts
...
- `muse gateway start` / `muse gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.hermes/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `muse gateway stop` stops only the current profile's gateway. `muse gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
