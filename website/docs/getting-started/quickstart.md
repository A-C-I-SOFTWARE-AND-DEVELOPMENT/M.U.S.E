...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with M.U.S.E. — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working M.U.S.E. setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [M.U.S.E. Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="M.U.S.E. Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up M.U.S.E. for a team, bot, or always-on workflow
...
|---|---|---|
| I just want M.U.S.E. working on my machine | `muse setup` | Run a real chat and verify it responds |
| I already know my provider | `muse model` | Save the config, then start chatting |
| I want a bot or always-on setup | `muse gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `muse model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `muse model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if M.U.S.E. cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `muse model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `muse model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `muse model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `muse model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `muse model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `muse model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `muse model` (requires `copilot` CLI + `copilot login`) |
...
M.U.S.E. requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `muse model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

M.U.S.E. separates secrets from normal config:
...
```bash
muse config set model anthropic/claude-opus-4.6
muse config set terminal.backend docker
muse config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
hermes            # classic CLI
muse --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
M.U.S.E. ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `muse` vs `muse --tui`.
:::
...
```bash
muse --continue    # Resume the most recent session
muse -c            # Short form
```
...
```bash
muse gateway setup    # Interactive platform configuration
```
...
- `muse tools` — tune tool access per platform
- `muse skills` — browse and install reusable workflows
...
```bash
muse config set terminal.backend docker    # Docker isolation
muse config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the M.U.S.E. install directory (the curl installer placed it at
...
|---|---|---|
| M.U.S.E. opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `muse model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `muse gateway setup` and check `muse gateway status` |
| `muse --continue` can't find old session | Switched profiles or session never saved | Check `muse sessions list` and confirm you're in the right profile |
...
| `muse doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `muse model`
3. `muse setup`
4. `muse sessions list`
5. `muse --continue`
6. `muse gateway status`
...
| `muse tools` | Configure which tools are enabled per platform |
| `muse setup` | Full setup wizard (configures everything at once) |
| `muse doctor` | Diagnose issues |
| `muse update` | Update to latest version |
| `muse gateway` | Start the messaging gateway |
| `muse --continue` | Resume last session |
