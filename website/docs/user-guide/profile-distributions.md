...
```bash
muse profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `muse profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
- **You just want to back up a profile on your own machine.** Use [`muse profile export` / `import`](../reference/profile-commands.md#muse-profile-export) — that's what those are for.
...
```bash
muse profile create research-bot
...
Recipients who run `muse profile update research-bot` will pull the latest.
...
```bash
muse profile install github.com/you/research-bot --alias
```
...
```bash
# GitHub shorthand
muse profile install github.com/you/research-bot

# Full HTTPS
muse profile install https://github.com/you/research-bot.git

# SSH
muse profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
muse profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
muse profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
muse profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
muse profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
muse profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
muse profile info research-bot
```
...
`muse profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
muse profile update research-bot
```
...
```bash
muse profile delete research-bot
```
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `muse profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
muse profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `muse profile update pr-reviewer` and everyone's on the new version within minutes.
...
# Anyone
muse profile install github.com/you/hermes-polymarket-trader --alias
```
...
# Each on-call
muse profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
muse profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `muse profile info <name>` and hold off on updates until you're ready.
:::
...
muse profile info research-bot | grep Version
...
muse profile delete research-bot --yes
muse profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
muse profile update research-bot --force-config --yes
```
...
muse profile install github.com/yourname/forked-research-bot --alias
...
muse profile install ~/.hermes/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
muse profile delete research-bot-test --yes
muse profile install ~/.hermes/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `muse -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `muse profile` — not a parallel command tree.
...
- [`muse profile export` / `import`](../reference/profile-commands.md#muse-profile-export) — local backup / restore (not distribution)
