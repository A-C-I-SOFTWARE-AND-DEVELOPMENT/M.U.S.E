...
muse --tui -r 20260409_000000_aa11bb
muse --tui --resume "my t0p session"
...
By default, `muse --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`muse --yolo`, `/yolo`, or `HERMES_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.
