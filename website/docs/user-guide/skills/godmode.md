...
The godmode skill integrates with two muse config mechanisms:
...
The `agent.system_prompt` is appended **after** muse's own system prompt — it augments, not replaces.
...
```bash
# Load the skill in a muse session
/godmode
...
6. **Restart muse after auto-jailbreak** — The CLI reads config once at startup. Gateway sessions pick up changes immediately.
