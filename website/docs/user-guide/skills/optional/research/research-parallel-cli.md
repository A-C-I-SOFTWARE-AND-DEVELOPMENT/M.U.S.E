...
| | |
|---|---|
| Source | Optional — install with `muse skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a M.U.S.E. core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with M.U.S.E. native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer M.U.S.E. native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer M.U.S.E. native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended M.U.S.E. usage patterns
...
- Do not silently choose Parallel when M.U.S.E. native tools are already sufficient.
