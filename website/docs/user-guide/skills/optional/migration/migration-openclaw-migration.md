---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into muse"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into muse"
---
...
Migrate a user's OpenClaw customization footprint into muse Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
# OpenClaw -> muse Migration

Use this skill when a user wants to move their OpenClaw setup into muse with minimal manual cleanup.
...
- import `SOUL.md` into the muse home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into muse memory entries
- merge OpenClaw command approval patterns into muse `command_allowlist`
...
- optionally copy the OpenClaw workspace instructions file into a chosen muse workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct muse destination
...
muse CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in muse How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a muse workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing muse skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of muse unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty muse targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean muse destination. Unsupported auth blobs must still be reported as skipped.
...
- muse persona state imported
- muse memory files populated with converted OpenClaw knowledge
