---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into M.U.S.E."
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into M.U.S.E."
---
...
Migrate a user's OpenClaw customization footprint into M.U.S.E.. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `muse skills install official/migration/openclaw-migration` |
...
# OpenClaw -> M.U.S.E. Migration

Use this skill when a user wants to move their OpenClaw setup into M.U.S.E. with minimal manual cleanup.
...
```bash
muse claw migrate              # Full interactive migration
...
muse claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `muse setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the M.U.S.E. home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into M.U.S.E. memory entries
- merge OpenClaw command approval patterns into M.U.S.E. `command_allowlist`
...
- optionally copy the OpenClaw workspace instructions file into a chosen M.U.S.E. workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct M.U.S.E. destination
...
M.U.S.E. CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in M.U.S.E.. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a M.U.S.E. workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing M.U.S.E. skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of M.U.S.E. unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty M.U.S.E. targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean M.U.S.E. destination. Unsupported auth blobs must still be reported as skipped.
...
- M.U.S.E. persona state imported
- M.U.S.E. memory files populated with converted OpenClaw knowledge
