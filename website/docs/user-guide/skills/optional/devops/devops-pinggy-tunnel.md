...
| | |
|---|---|
| Source | Optional — install with `muse skills install official/devops/pinggy-tunnel` |
| Path | `optional-skills/devops/pinggy-tunnel` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), M.U.S.E. |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. M.U.S.E.' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
