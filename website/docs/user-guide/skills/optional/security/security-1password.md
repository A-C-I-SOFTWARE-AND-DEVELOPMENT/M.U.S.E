...
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by muse |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during muse terminal calls (desktop app flow only)
...
### Service Account (recommended for muse)
...
## muse Execution Pattern (desktop app flow)

muse terminal commands are non-interactive by default and can lose auth context between calls.
