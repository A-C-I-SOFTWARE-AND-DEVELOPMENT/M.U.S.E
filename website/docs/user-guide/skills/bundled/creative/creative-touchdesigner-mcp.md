...
```
M.U.S.E. -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. Add `twozero_td` MCP server to M.U.S.E. config (if missing)
...
3. **Restart M.U.S.E. session** to pick up the new MCP server
