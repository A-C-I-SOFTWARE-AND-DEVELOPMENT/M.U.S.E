...
- A long-running process (muse gateway, tui_gateway) misbehaves and you can't restart it
...
For long-lived processes: M.U.S.E. gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside M.U.S.E. you have two practical options:
...
### `tui_gateway` subprocess (spawned by `muse --tui`)
...
debugpy.wait_for_client()
```
Start `muse --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For M.U.S.E. subagents, debug one process at a time.
