...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger M.U.S.E. agent runs by POSTing events to a URL.
...
All management is via the `muse webhook` CLI command:

### Create a subscription
```bash
muse webhook subscribe <name> \
...
```bash
muse webhook remove <name>
```

### Test a subscription
```bash
muse webhook test <name>
muse webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
muse webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
muse webhook subscribe stripe-payments \
...
```bash
muse webhook subscribe ci-builds \
  --events "pipeline" \
...
1. `muse webhook subscribe` writes to `~/.hermes/webhook_subscriptions.json`
...
4. **Signature mismatch?** Verify the secret in your service matches the one from `muse webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `muse webhook test <name>` to verify the route works.
