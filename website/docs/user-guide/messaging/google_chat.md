...
sidebar_position: 12
title: "Google Chat"
description: "Set up M.U.S.E. as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect M.U.S.E. to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your M.U.S.E.
...
download the file. Save it somewhere only M.U.S.E. can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — M.U.S.E. calls
...
- **App name**: whatever you want users to see ("M.U.S.E." is reasonable).
...
`ADDED_TO_SPACE` event that M.U.S.E. uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure M.U.S.E.
...
Install the dependencies the Google Chat adapter needs (no M.U.S.E. extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "M.U.S.E. is thinking…" marker, then
...
Thread support: when a user replies inside a thread, M.U.S.E. detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate M.U.S.E. session.
...
3. Download the JSON. Move it onto the host that runs M.U.S.E..
4. On the host, register the client with M.U.S.E.:
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, M.U.S.E. isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: M.U.S.E. will only attach the SA bearer
