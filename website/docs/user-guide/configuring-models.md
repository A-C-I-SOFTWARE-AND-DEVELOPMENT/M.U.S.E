...
M.U.S.E. uses two kinds of model slots:
...
- **Right** — the curated model list for the selected provider. These are the agentic models M.U.S.E. recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and M.U.S.E. writes it to `~/.hermes/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning M.U.S.E. uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `muse skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, M.U.S.E. writes to `~/.hermes/config.yaml`:
...
`provider: auto` with `model: ''` tells M.U.S.E. to use the main model for that task.

## When does it take effect?

- **CLI** (`muse chat`): next `muse chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`muse gateway restart`) if you want to force all sessions to pick up the change.
...
M.U.S.E. lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `muse setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but M.U.S.E. switched providers on me
...
```bash
muse config set model.aliases.fav anthropic/claude-opus-4.6
muse config set model.aliases.grok x-ai/grok-4
```
...
### `muse model` subcommand

```bash
muse model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`muse model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.hermes/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `muse config get model` and `muse status`.
