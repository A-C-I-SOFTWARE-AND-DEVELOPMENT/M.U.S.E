...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `muse memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `muse plugins list` and the interactive `muse plugins` UI), but none load until you explicitly enable them:

```bash
muse plugins enable disk-cleanup
```
...
```bash
muse plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `muse plugins enable <name>`.
...
**Enabling:** `muse plugins enable disk-cleanup` (or check the box in `muse plugins`).

**Disabling again:** `muse plugins disable disk-cleanup`.
...
Or check the box in the interactive `muse plugins` UI. Then put the credentials in `~/.hermes/.env`:
...
**Disabling:** `muse plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
```bash
muse plugins enable google_meet
...
**Disabling:** `muse plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.hermes/cache/google_meet/` until you remove them.
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `muse dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.
