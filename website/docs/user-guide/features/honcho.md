...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of M.U.S.E.'s built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple M.U.S.E. instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
muse memory setup    # select "honcho" from the provider list
```
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — M.U.S.E. syncs them back at session init.
...
The `muse honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `muse memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
muse honcho status          # Connection status, config, and key settings
muse honcho setup           # Redirects to `muse memory setup`
muse honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
muse honcho peer            # Show or update peer names + dialectic reasoning level
muse honcho mode            # Show or set recall mode (hybrid/context/tools)
muse honcho tokens          # Show or set token budget for context and dialectic
muse honcho identity        # Seed or show the AI peer's Honcho identity
muse honcho sync            # Sync Honcho config to all existing profiles
muse honcho peers           # Show peer identities across all profiles
muse honcho sessions        # List known Honcho session mappings
muse honcho map             # Map current directory to a Honcho session name
muse honcho enable          # Enable Honcho for the active profile
muse honcho disable         # Disable Honcho for the active profile
muse honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `muse honcho`

If you previously used the standalone `muse honcho setup`:
...
No re-login or re-setup needed. Run `muse memory setup` and select "honcho" — the wizard detects your existing config.
