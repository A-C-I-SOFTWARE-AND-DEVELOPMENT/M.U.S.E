...
description: "Durable SQLite-backed task board for coordinating multiple M.U.S.E. profiles"
---
...
M.U.S.E. Kanban is a durable task board, shared across all your M.U.S.E. profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.hermes/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.hermes/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `muse kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `muse kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
muse kanban boards list

# Create a new board.
muse kanban boards create atm10-server \
    --name "ATM10 Server" \
...
muse kanban --board atm10-server list
muse kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
muse kanban boards switch atm10-server
muse kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
muse kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.hermes/kanban/current` — the slug persisted by `muse kanban
   boards switch`.
4. `default`.
...
`muse dashboard` → Kanban tab shows a board switcher at the top as soon
...
muse kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `muse kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `muse gateway
...
until one comes up — `muse kanban create` warns about this at creation
time.

Running `muse kanban daemon` as a separate process is **deprecated**;
...
muse kanban create "nightly ops review" \
    --assignee ops \
...
```bash
muse kanban complete t_abc t_def t_hij --result "batch wrap"
muse kanban archive  t_abc t_def t_hij
muse kanban unblock  t_abc t_def
muse kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `muse kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `muse kanban` CLI.
...
**Zero schema footprint on normal sessions.** A regular `muse chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```bash
muse -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
muse -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
muse kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

muse kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
muse -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
muse -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `muse kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `muse kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `muse profile create --description "..."`, `muse profile describe <name> --text "..."`, `muse profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `muse profile describe --auto`). |
...
If you run `muse dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.hermes/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `muse -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard M.U.S.E. dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
muse kanban init                                     # create kanban.db + print daemon hint
muse kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
muse kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
muse kanban show <id> [--json]
muse kanban assign <id> <profile>                    # or 'none' to unassign
muse kanban link <parent_id> <child_id>
muse kanban unlink <parent_id> <child_id>
muse kanban claim <id> [--ttl SECONDS]
muse kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
muse kanban complete <id>... [--result "..."]
muse kanban block <id> "<reason>" [--ids <id>...]
muse kanban unblock <id>...
muse kanban archive <id>...

muse kanban tail <id>                                # follow a single task's event stream
muse kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
muse kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
muse kanban runs <id> [--json]                       # attempt history (one row per run)
muse kanban assignees [--json]                       # profiles on disk + per-assignee task counts
muse kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
muse kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `muse gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
muse kanban stats [--json]                           # per-status + per-assignee counts
muse kanban log <id> [--tail BYTES]                  # worker log from ~/.hermes/kanban/logs/
muse kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
muse kanban notify-list [<id>] [--json]
muse kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
muse kanban context <id>                             # what a worker sees
muse kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
muse kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `muse kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `muse chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `muse kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `muse kanban`. You don't have to leave the chat to drive the board.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`muse kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `muse kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
```bash
muse kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
muse kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
muse kanban notify-list
muse kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
muse kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
muse kanban runs t_abcd
...
**Bulk close caveat.** `muse kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`muse kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `muse kanban heartbeat $TASK` to signal liveness during long operations. |
...
`muse kanban tail <id>` shows these for a single task. `muse kanban watch` streams them board-wide.
