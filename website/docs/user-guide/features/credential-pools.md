...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, M.U.S.E. automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, M.U.S.E. auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
muse auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
muse auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
muse auth add anthropic --type oauth
...
Run `muse auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `muse auth` | Interactive pool management wizard |
| `muse auth list` | Show all pools and credentials |
| `muse auth list <provider>` | Show a specific provider's pool |
| `muse auth add <provider>` | Add a credential (prompts for type and key) |
| `muse auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `muse auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `muse auth remove <provider> <index>` | Remove credential by 1-based index |
| `muse auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `muse auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `muse model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via muse model:
muse auth list
# Shows:
...
muse auth add Together.ai --api-key sk-together-second-key
```
...
M.U.S.E. automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| M.U.S.E. PKCE OAuth | `~/.hermes/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `muse auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `muse auth add`) are never auto-pruned.
