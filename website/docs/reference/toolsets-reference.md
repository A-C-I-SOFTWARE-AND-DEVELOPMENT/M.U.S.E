...
description: "Reference for M.U.S.E. core, composite, platform, and dynamic toolsets"
---
...
```bash
muse chat --toolsets web,file,terminal
muse chat --toolsets debugging        # composite — expands to file + terminal + web
muse chat --toolsets all              # everything
```
...
```bash
muse tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call M.U.S.E. tools programmatically. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `muse tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
...
## Relationship to `muse tools`

The `muse tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
