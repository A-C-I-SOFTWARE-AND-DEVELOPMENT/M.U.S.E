...
```bash
muse config set model.default grok-4.3
muse config set model.provider xai-oauth
```
...
```bash
muse --provider xai-oauth        # canonical
muse --provider grok-oauth       # alias
muse --provider x-ai-oauth       # alias
muse --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `muse tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `muse tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
This is **not** a stale-token problem — re-running `muse model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/hermes-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
muse config set model.provider xai
```
...
**Fix:** run `muse model` and pick the xAI Grok OAuth provider, or run `muse auth add xai-oauth`.
...
```bash
muse auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `muse auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `muse auth list xai-oauth` to see them).
