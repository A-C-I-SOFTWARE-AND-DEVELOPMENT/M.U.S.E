...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `muse send`."
---

# Pipe Script Output to Messaging Platforms

`muse send` is a small, scriptable CLI that pushes a message to any
messaging platform M.U.S.E. is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | muse send --to slack:#builds`)
...
muse send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | muse send --to telegram:-1001234567890

# Send a file
muse send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
muse send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
muse send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
muse send --list

# Filter by platform
muse send --list telegram
```
...
Any platform M.U.S.E. ships adapters for works as a target:
...
`muse send` resolves the message body in this order:

1. **Positional argument** — `muse send --to telegram "hi"`
2. **`--file PATH`** — `muse send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | muse send --to telegram`

When stdin is a TTY (no pipe), M.U.S.E. does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  muse send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `muse send` reuses your M.U.S.E. config, the same script works on
any host where M.U.S.E. is installed — no need to export bot tokens into
...
`muse send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  muse send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | muse send \
...
```bash
./train.py --epochs 200 && \
  muse send --to telegram "training done" || \
  muse send --to telegram "training failed (exit $?)"
```
...
muse send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(muse send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `muse send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `muse send` calls
...
pointing at the gateway; start it with `muse gateway start` and retry.
...
"no channels discovered yet", start the gateway once (`muse gateway
...
| Approach | Multi-platform | Reuses M.U.S.E. creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `muse send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`muse send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `muse send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `muse send` shares with cron delivery.
