...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/A-C-I-SOFTWARE-AND-DEVELOPMENT/M.U.S.E/blob/main/{source_dir}/{meta['rel_path']}"
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with MUSE"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "MUSE ships with a large built-in skill library copied into `~/.muse/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "MUSE also syncs bundled skills on `muse update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.muse/skills/` tree, it is still shipped with MUSE; restore it with `muse skills reset <name> --restore`.",
        "",
