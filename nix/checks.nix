    ...
    in {
      ...
      checks = {
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${hermes-agent}/bin/muse || (echo "FAIL: muse binary missing"; exit 1)
          test -x ${hermes-agent}/bin/muse-agent || (echo "FAIL: muse-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${hermes-agent}/bin/muse version 2>&1 | grep -qi "m\.u\.s\.e" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in muse muse-agent muse-acp; do
        ...
        cli-commands = pkgs.runCommand "hermes-cli-commands" { } ''
          ...
          echo "=== Checking muse --help ==="
          ${hermes-agent}/bin/muse --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${hermes-agent}/bin/muse --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          grep -q "HERMES_BUNDLED_SKILLS" ${hermes-agent}/bin/muse || \
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          grep -q "HERMES_BUNDLED_PLUGINS" ${hermes-agent}/bin/muse || \
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          grep -q "HERMES_TUI_DIR" ${hermes-agent}/bin/muse || \
        ...
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          ...
          grep -q "HERMES_NODE" ${hermes-agent}/bin/muse || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${hermes-agent}/bin/muse)
        ...
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked "config set" ${hermes-agent}/bin/muse config set model foo
          check_blocked "config edit" ${hermes-agent}/bin/muse config edit
        ...
        in pkgs.runCommand "hermes-extra-python-packages" { } ''
          ...
          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/muse || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/muse || \
          ...
          if grep -q "PYTHONPATH" ${hermes-agent}/bin/muse; then
