...
class SettingsRepository(private val context: Context) {
    ...
    val interactiveIconEnabled: Flow<Boolean> =
        context.dataStore.data.map { it[Keys.INTERACTIVE_ICON_ENABLED] ?: true }
    val gatewayEndpoint: Flow<String> =
        context.dataStore.data.map { it[Keys.GATEWAY_ENDPOINT] ?: DEFAULT_GATEWAY_ENDPOINT }
    val mockMode: Flow<Boolean> = context.dataStore.data.map { it[Keys.MOCK_MODE] ?: false }
    val termuxGatewayMode: Flow<Boolean> =
        context.dataStore.data.map { it[Keys.TERMUX_GATEWAY_MODE] ?: false }
    val approvalsRequired: Flow<Boolean> =
        context.dataStore.data.map { it[Keys.APPROVALS_REQUIRED] ?: true }
    val safetyGatesEnabled: Flow<Boolean> =
        context.dataStore.data.map { it[Keys.SAFETY_GATES_ENABLED] ?: true }
    val privacyLocalOnlyMemory: Flow<Boolean> =
        context.dataStore.data.map { it[Keys.PRIVACY_LOCAL_ONLY_MEMORY] ?: true }
    val emergencyStopEngaged: Flow<Boolean> =
        context.dataStore.data.map { it[Keys.EMERGENCY_STOP_ENGAGED] ?: false }
    ...
    suspend fun setGatewayEndpoint(value: String) {
        context.dataStore.edit { it[Keys.GATEWAY_ENDPOINT] = value.trim() }
    }
    ...
    companion object {
        const val DEFAULT_GATEWAY_ENDPOINT = "http://127.0.0.1:8765"
    }
}
