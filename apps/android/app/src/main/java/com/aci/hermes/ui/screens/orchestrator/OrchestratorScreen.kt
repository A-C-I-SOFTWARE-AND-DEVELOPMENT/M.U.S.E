...
fun OrchestratorScreen(
    ...
    onOpenEmergencyControl: () -> Unit,
    onEngageEmergencyStop: () -> Unit,
    ...
    Scaffold(
            ...
            TopAppBar(
                ...
                actions = {
                    EmergencyStopButton(
                        ...
                        onTap = {
                            if (state.emergencyStop == EmergencyStopState.INACTIVE) {
                                onEngageEmergencyStop()
                            } else {
                                onOpenEmergencyControl()
                            }
                        },
                        onLongPressEscalate = onLongPressEscalate,
        ...
        floatingActionButton = {
            ExtendedFloatingActionButton(
                onClick = {
                    if (viewModel.guardNewTask()) onOpenTask(null)
                },
        ...
        LazyColumn(
            ...
            if (state.emergencyStop != EmergencyStopState.INACTIVE) {
                item {
                    EmergencyStopBanner(
                        state = state.emergencyStop,
                        onOpenControl = onOpenEmergencyControl,
                    )
                }
            }
            item {
                CriticalActionCard(
                    state = state.emergencyStop,
                    onEngageStop = onEngageEmergencyStop,
                    onEscalate = onLongPressEscalate,
                    onRequestResume = onOpenEmergencyControl,
                    onOpenControl = onOpenEmergencyControl,
                )
            }
