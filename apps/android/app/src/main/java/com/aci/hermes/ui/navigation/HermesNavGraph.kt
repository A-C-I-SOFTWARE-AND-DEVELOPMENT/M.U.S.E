...
import com.aci.hermes.ui.screens.jarvis.JarvisChatScreen
    ...
    NavHost(navController = nav, startDestination = Screen.Splash.route) {
        ...
        composable(Screen.Orchestrator.route) {
            ...
            OrchestratorScreen(
                ...
                onOpenJarvisChat = { nav.navigate(Screen.JarvisChat.route) },
            )
        }
        composable(Screen.JarvisChat.route) {
            val vm: JarvisChatViewModel = viewModel(factory = remember { container.jarvisChatVmFactory() })
            JarvisChatScreen(viewModel = vm, onBack = { nav.popBackStack() })
        }
        composable(
