...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted muse & Nous-trained models',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when muse needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before muse stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters muse can read from one file request.',
  approvals: {
    mode: 'How muse handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When muse updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
