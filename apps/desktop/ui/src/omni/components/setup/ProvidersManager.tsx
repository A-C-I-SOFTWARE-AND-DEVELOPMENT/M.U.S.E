    ...
    <div className="glass overflow-hidden">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between px-3 py-2.5 text-left">
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full" style={{ background: configured ? 'var(--state-auth)' : 'var(--ink-faint)' }} />
          <div>
...
export function ProvidersManager() {
  ...
  const configured = PROVIDERS.filter((p) => isConfigured(p));
    ...
    <div className="flex flex-col gap-3">
      ...
      {configured.length > 0 && (
        <div className="glass px-3 py-2.5">
          <div className="hud-label mb-1.5">Configured · {configured.length}</div>
          <div className="flex flex-wrap gap-1.5">
            {configured.map((p) => (
              <span key={p.id} className="mono rounded-full px-2 py-0.5 text-[10px]" style={{ background: 'rgba(255,184,77,0.12)', color: 'var(--state-auth)' }}>{p.label}</span>
            ))}
          </div>
        </div>
      )}
