...
# in lockstep with muse_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``muse_cli.__version__`` is
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``muse_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``muse_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from muse_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[Any], Optional[str]]:
    ...
    """
    try:
        from muse_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from muse_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from muse_cli.models import copilot_default_headers
...
def _try_azure_foundry(
    *,
    ...
    :func:`muse_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from muse_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _to_async_client(sync_client, model: Optional[str], is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from muse_cli.copilot_auth import copilot_request_headers
...
def resolve_provider_client(
    ...
    try:
        from muse_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("muse_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from muse_cli.models import _should_use_copilot_responses_api
