...
DEFAULT_AGENT_IDENTITY = (
    "You are muse (Multi-Use Synaptic Entity), an intelligent AI operating partner built on the Hermes Agent runtime by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on muse (built on the Hermes Agent runtime by Nous Research). When the user needs help with "
    "muse itself — configuring, setting up, using, extending, or troubleshooting "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that muse "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the M.U.S.E. terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the M.U.S.E. desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the M.U.S.E. WebUI, a browser-based chat interface. "
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where muse itself is running. The host OS, home, and cwd "
                f"of the muse process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where muse "
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot muse itself — its CLI, config, models, providers, tools, "
...
def build_context_files_prompt(
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the muse install tree (%s) — set terminal.cwd to "
