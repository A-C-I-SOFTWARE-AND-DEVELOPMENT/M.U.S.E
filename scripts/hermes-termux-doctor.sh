...
#   bash scripts/hermes-termux-doctor.sh --quiet     # only failures
#   bash scripts/hermes-termux-doctor.sh --json      # machine-readable
# ============================================================================
...
if is_termux; then
    ...
    _print info "This script is designed for Termux on Android" \
        "see docs/termux/hermes-phone-first-runtime.md"
fi
...
if have termux-wake-lock && have termux-wake-unlock; then
    ...
    _print info "Wake lock keeps the CPU awake while the gateway runs in the background" \
        "see docs/termux/hermes-android-permissions.md"
else
