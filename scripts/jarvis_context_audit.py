#!/usr/bin/env python3
"""Audit the JARVIS Prime operating-layer files.
...
def audit() -> int:
    ...
    if readme.exists():
        ...
        check("JARVIS Prime" in readme_text, "README.md JARVIS integration detected", failures, warnings, warn=True)
    else:
        check(False, "README.md exists", failures, warnings, warn=True)
    if agents.exists():
        agents_text = agents.read_text(encoding="utf-8")
        check("JARVIS Prime Operating Layer" in agents_text, "AGENTS.md JARVIS integration detected", failures, warnings, warn=True)
    else:
