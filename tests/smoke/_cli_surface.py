"""Build the *complete* ``muse`` argparse tree without dispatching a command.
...
every subparser.  Running ``main()`` with ``argv = ["muse", "--help"]``
...
def build_cli_parser():
    """Return ``(top_level_parser, subparsers_action)`` for the muse CLI."""
    ...
    try:
        ...
        if "subparsers" not in captured:
            raise CliSurfaceError(
                f"muse CLI parser tree could not be built: "
