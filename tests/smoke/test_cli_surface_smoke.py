"""Every registered ``muse`` command resolves and renders its ``--help``.
...
* ``parser.format_help()`` renders — which is what argparse does for
  ``muse <command> --help``, and is where a bad ``%(default)s`` in a help
...
def test_cli_parser_tree_builds() -> None:
    ...
    assert _BUILD_ERROR is None, (
        f"the muse CLI parser tree could not be built, so no command could "
...
def test_cli_exposes_a_realistic_command_surface() -> None:
    ...
    assert len(_TOP_LEVEL) >= 40, (
        f"only {len(_TOP_LEVEL)} top-level muse commands were discovered; "
...
def test_command_help_renders(path: tuple[str, ...]) -> None:
    """``muse <path> --help`` must format without raising."""
    parser = parser_for(path)
    help_text = parser.format_help()
    assert help_text.strip(), f"muse {' '.join(path)} --help rendered nothing"
    assert "usage" in help_text.lower(), (
        f"muse {' '.join(path)} --help produced no usage line:\n{help_text[:400]}"
    )
...
def test_top_level_command_resolves_to_first_party_callable(name: str) -> None:
    ...
    assert handler is not None, (
        f"muse {name} registers no `func` default, so main() has nothing to "
        f"dispatch to."
    )
    assert callable(handler), f"muse {name} -> func is not callable: {handler!r}"
    origin = getattr(handler, "__module__", "") or ""
    root = origin.split(".")[0]
    assert root in first_party_top_level(), (
        f"muse {name} dispatches to {origin}.{getattr(handler, '__name__', handler)!r}, "
...
def test_subcommand_func_default_is_callable_when_declared(
    ...
    if handler is None:
        pytest.skip(
            f"muse {' '.join(path)} dispatches on its `dest` string rather "
            f"than a `func` default; there is no handler object to check."
        )
    assert callable(handler), (
        f"muse {' '.join(path)} -> func is not callable: {handler!r}"
    )
