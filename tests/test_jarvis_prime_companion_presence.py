...
def test_target_screen_routes_to_turning_page() -> None:
    policy = CompanionPresencePolicy()
    state = policy.state_for(PresenceSignals(target_app="Facebook", target_on_next_screen=True))
    assert state is PresenceState.TURNING_PAGE
