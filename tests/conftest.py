...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    try:
        import muse_cli.plugins as _plugins_mod
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "muse_cli"):
...
# correlated with a ``tests/muse_cli/`` pytest run starting up.
#
...
def _live_system_guard(request, monkeypatch):
    ...
    _HERMES_TOKENS = (
        ...
        # Legacy pre-rename cmdlines — a live gateway started before the
        # hermes_cli -> muse_cli rename still matches these. Keep both.
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m muse_cli.main".
                if (
                    "hermes" in low
                    or "muse" in low
                    or "gateway" in low
