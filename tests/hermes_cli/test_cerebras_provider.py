"""Tests for Cerebras provider support — standard direct API provider."""
...
class TestCerebrasAliases:
    @pytest.mark.parametrize("alias", ["cerebras", "cerebras-ai", "cerebrasai", "cerebras-cloud"])
    def test_alias_resolves(self, alias, monkeypatch):
        for key in _OTHER_PROVIDER_KEYS + ("OPENROUTER_API_KEY",):
            monkeypatch.delenv(key, raising=False)
        monkeypatch.setenv("CEREBRAS_API_KEY", "csk-test-12345")
        assert resolve_provider(alias) == "cerebras"

    def test_normalize_provider_models_py(self):
        from hermes_cli.models import normalize_provider
        assert normalize_provider("cerebras-ai") == "cerebras"
        assert normalize_provider("cerebrasai") == "cerebras"
...
class TestCerebrasModelCatalog:
    def test_static_model_list(self):
        """Cerebras has a static _PROVIDER_MODELS catalog entry. Specific model
        names rotate fast on Cerebras (May 2026 deprecated four IDs) and don't
        belong in tests.
        """
...
class TestCerebrasURLMapping:
    ...
    def test_provider_prefixes(self):
        ...
        assert "cerebras-ai" in _PROVIDER_PREFIXES
        assert "cerebrasai" in _PROVIDER_PREFIXES
...
# =============================================================================
# Auxiliary client — aux model comes from the profile, not the legacy dict
# =============================================================================


class TestCerebrasAuxiliary:
    def test_no_legacy_fallback_entry(self):
        """New providers set default_aux_model on the profile — the legacy
        fallback dict must stay untouched."""
        from agent.auxiliary_client import _API_KEY_PROVIDER_AUX_MODELS
        assert "cerebras" not in _API_KEY_PROVIDER_AUX_MODELS

    def test_profile_default_aux_model(self):
