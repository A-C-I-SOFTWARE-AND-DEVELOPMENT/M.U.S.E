...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("muse_cli.setup.ensure_hermes_home", {}),
        ("muse_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("muse_cli.config.is_managed", {"return_value": False}),
        ("muse_cli.setup.load_config", {"return_value": {}}),
        ("muse_cli.setup.save_config", {}),
        ("muse_cli.setup.get_env_value", {"return_value": None}),
        ("muse_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("muse_cli.setup._print_setup_summary", {}),
        ("muse_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("muse_cli.setup.load_config", {"return_value": {}}),
        ("muse_cli.setup.save_config", {}),
        ("muse_cli.auth.get_active_provider", {"return_value": None}),
        ("muse_cli.setup.get_env_value", {"return_value": None}),
        ("muse_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))  # ty: ignore[no-matching-overload]  # mock/duck-typed test fixture
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="muse_cli.setup.prompt_choice",
                quick="muse_cli.setup._run_quick_setup",
                model="muse_cli.setup.setup_model_provider",
                terminal="muse_cli.setup.setup_terminal_backend",
                agent="muse_cli.setup.setup_agent_settings",
                gateway="muse_cli.setup.setup_gateway",
                tools="muse_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("muse_cli.setup.prompt_choice", {"return_value": 0}),
                first="muse_cli.setup._run_first_time_quick_setup",
            )
            from muse_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "muse_cli.setup.run_setup_wizard",
