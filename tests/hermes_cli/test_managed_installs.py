...
def test_recommended_update_command_defaults_to_muse_update(monkeypatch):
    ...
    with patch("muse_cli.config.get_managed_update_command", return_value=None), \
         patch("muse_cli.config.detect_install_method", return_value="git"):
        assert recommended_update_command() == "muse update"


def test_cmd_update_blocks_managed_homebrew(monkeypatch, capsys):
    monkeypatch.setenv("HERMES_MANAGED", "homebrew")

    with patch("muse_cli.main.subprocess.run") as mock_run:
