...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "muse_cli"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("muse_cli.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "muse_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    muse_cli = types.ModuleType("muse_cli")
    muse_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["muse_cli"] = muse_cli
    sys.modules["muse_cli.config"] = types.SimpleNamespace(  # ty: ignore[invalid-assignment]
