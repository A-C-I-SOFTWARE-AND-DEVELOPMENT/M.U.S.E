...
@pytest.mark.skip(
    reason=(
        "Requires the paired tools/file_tools.py change that pre-resolves "
        "relative paths against TERMINAL_CWD before calling get_read_block_error. "
        "That change is out of scope for this PR (file is in the M-set with "
        "significant ACI divergence) and will land in a follow-up reconciliation "
        "PR per the file_safety docstring at lines ~203-208."
    )
)
