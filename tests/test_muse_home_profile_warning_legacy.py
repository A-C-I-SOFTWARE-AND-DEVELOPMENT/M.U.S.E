...
def fresh_constants(monkeypatch, tmp_path):
    """Import muse_constants fresh and reset the one-shot warn flag."""
    import importlib
    import muse_constants
    importlib.reload(muse_constants)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("HERMES_HOME", raising=False)
    return muse_constants
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: nothing on disk → silent, returns the canonical
        fresh-install default ~/.muse (post-rename invariant)."""
        result = fresh_constants.get_hermes_home()
        assert result == tmp_path / ".muse"
