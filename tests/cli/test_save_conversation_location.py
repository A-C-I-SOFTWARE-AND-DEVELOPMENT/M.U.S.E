...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(muse_constants, "_hermes_home_cache"):
        muse_constants._hermes_home_cache = None
    return home
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    ...
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "muse_constants"]:
