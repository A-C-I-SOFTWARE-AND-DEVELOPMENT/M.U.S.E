...
def test_owner_gated_actions_includes_known_categories() -> None:
    for action in (
        "spend_money", "production_deploy", "main_branch_merge",
        "package_publish", "credential_change", "regulated_claim",
    ):
...
def test_request_then_no_authorize_means_no_grant() -> None:
    auth = OwnerAuth()
    auth.request("main_branch_merge", risk_class="RC2", rationale="ship sync PR")
    assert auth.pending_actions() == ["main_branch_merge"]
...
def test_exact_phrase_grants_authorization() -> None:
    auth = OwnerAuth()
    auth.request("main_branch_merge", rationale="ship")
    granted = auth.authorize(AUTHORIZATION_PHRASE)
    assert len(granted) == 1
    assert granted[0].action == "main_branch_merge"
