...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['muse_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("muse_state")
    fake_module.SessionDB = lambda: fake_db  # ty: ignore[unresolved-attribute]  # mock/duck-typed test fixture
    plugin_api._test_monkeypatch.setitem(sys.modules, "muse_state", fake_module)
