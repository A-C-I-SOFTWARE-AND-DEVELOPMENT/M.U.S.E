...
def test_offline_run_does_not_leak_process_environment(monkeypatch, pipeline_root):
    monkeypatch.delenv("AXIOM_STUDIO_OFFLINE", raising=False)
    ...
    AAAPipeline(pipeline_root).run(brief)

    assert "AXIOM_STUDIO_OFFLINE" not in os.environ
...
def test_ue5_generator_writes_substantive_project(tmp_path):
    ...
    pcg = json.loads(
        (tmp_path / "ue5" / "Config" / "PCGGraphs.json").read_text(encoding="utf-8")
    )
    assert pcg["foliage_scatter"]["instances_per_km2"] == (
        profile.density.foliage_instances_per_km2
    )
    assert pcg["foliage_scatter"]["max_instances"] == (
