"""Tests for muse_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Hermes entry point
(muse, muse-agent, muse-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports muse_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported muse_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("muse_bootstrap", None)
    import muse_bootstrap  # noqa: WPS433
    return muse_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import muse_bootstrap as its
    ...
    # import muse_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import muse_bootstrap' and that
        ...
        half-finished ``muse update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``muse_bootstrap`` as a top-level module) leaves muse
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # muse start even when muse_bootstrap hasn't been
        ...
        assert first_import_name == "muse_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'muse_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import muse_bootstrap' line to be the first import."
        )
