...
HAND: Dict[str, Tuple[str, str]] = {
    ...
    "scripts/fleet/provision_user.sh:66": (
        "not_a_credential",
        "shell command substitution reading the cockpit token out of a running "
        "container at provisioning time; the regex captured the text up to an "
        "inner quote, so no literal is present",
    ),
    "apps/android/app/src/main/java/com/aci/hermes/data/audit/AuditRepository.kt:482": (
        "test_fixture",
        "hard-coded DefaultMockAuditSeed preview record for the Android audit "
        "UI; production wires EmptyAuditSeed, and the record is additionally "
        "passed through redactedForDisplay()",
    ),
    ...
    "tools/security/tests/test_pickle_site_adoption.py": (
        "test_fixture",
        "fixtures for the three 9.1 pickle load sites; no credential-shaped "
        "literal in this file is a credential",
    ),
}
