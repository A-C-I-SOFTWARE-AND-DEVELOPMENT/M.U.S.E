...
2. **Record-on-first-use, loudly.** The first time an artifact is seen with no
   ...
   Set ``MUSE_PICKLE_PINS_STRICT=1`` (or pass ``allow_record_on_first_use=
...
3. **Refuse thereafter.** Once a pin exists it is authoritative. A changed
   ...
   silently. Re-pinning is a deliberate act (``muse-pin --repin`` below, or
...
PIN_FILE_NAME = ".muse-pickle-pins.json"
PIN_FILE_VERSION = 1

_STRICT_ENV = "MUSE_PICKLE_PINS_STRICT"
...
def _default_store_for(path: Path) -> PinStore:
    override = os.environ.get("MUSE_PICKLE_PINS_FILE", "").strip()
    if override:
...
def safe_pickle_load(
    ...
    **pickle_kwargs: Any,
    ...
    store:
        Where pins live. Defaults to ``<artifact dir>/.muse-pickle-pins.json``,
        overridable with ``MUSE_PICKLE_PINS_FILE``.
    allow_record_on_first_use:
        ``None`` (default) honours ``MUSE_PICKLE_PINS_STRICT``. ``False``
