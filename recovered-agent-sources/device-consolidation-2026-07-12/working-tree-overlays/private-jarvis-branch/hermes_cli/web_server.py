...
app = FastAPI(title="JARVIS Prime", version=__version__)
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "jarvis-prime-update": "jarvis-prime-update.log",
}
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn a JARVIS Prime subcommand detached and record the Popen handle.
...
async def update_hermes():
    """Kick off ``jarvis-prime update`` in the background."""
    try:
        proc = _spawn_hermes_action(["update"], "jarvis-prime-update")
    except Exception as exc:
        _log.exception("Failed to spawn JARVIS Prime update")
        raise HTTPException(status_code=500, detail=f"Failed to start update: {exc}")
    return {
        "ok": True,
        "pid": proc.pid,
        "name": "jarvis-prime-update",
    }
