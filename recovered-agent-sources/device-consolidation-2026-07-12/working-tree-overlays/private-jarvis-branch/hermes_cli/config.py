"""
Configuration management for JARVIS Prime.
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade jarvis-prime"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade jarvis-prime"
        return "pip install --upgrade jarvis-prime"
    return "jarvis-prime update"
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this JARVIS Prime installation is managed by NixOS "
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this JARVIS Prime installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade jarvis-prime"
        )

    return (
        f"Cannot {action}: this JARVIS Prime installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall JARVIS Prime."
    )
...
DEFAULT_CONFIG = {
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *JARVIS Prime*" header.
    ...
    # ``jarvis-prime update`` behaviour.
    "updates": {
        # Run a full ``hermes backup``-style zip of HERMES_HOME before every
        # ``jarvis-prime update``.  Backups land in ``<HERMES_HOME>/backups/`` and
...
OPTIONAL_ENV_VARS = {
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'jarvis-prime' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `jarvis-prime update`.  Skill-config prompting is a
...
def show_config():
    ...
    print(color("│              JARVIS Prime Configuration                 │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
