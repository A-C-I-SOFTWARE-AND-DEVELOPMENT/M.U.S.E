...
AUTHOR_MAP = {
    ...
    "echerd27@gmail.com": "echerd27-design",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"jarvis-prime[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/A-C-I-SOFTWARE-AND-DEVELOPMENT/hermes-agent",
    ...
    lines.append(f"# JARVIS Prime v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for JARVIS Prime. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="JARVIS Prime Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  JARVIS Prime Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"JARVIS Prime v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"JARVIS Prime v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'JARVIS Prime v{new_version} ({calver_date})' "
