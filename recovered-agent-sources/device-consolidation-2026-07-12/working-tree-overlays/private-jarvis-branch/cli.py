#!/usr/bin/env python3
"""
JARVIS Prime CLI - Interactive Terminal Interface

A command-line interface for JARVIS Prime, inspired by Claude Code.
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "JARVIS PRIME - AI Operating Partner"
        tiny_line = "JARVIS PRIME"
    else:
        agent_name = _skin.get_branding("agent_name", "JARVIS Prime") if _skin else "JARVIS Prime"
...
class HermesCLI:
    """
    Interactive CLI for JARVIS Prime.
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from hermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ JARVIS")
    ...
    def show_banner(self):
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with JARVIS Prime.[/]"
            )
            self._console_print(
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update JARVIS Prime to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``jarvis-prime update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update JARVIS Prime')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update JARVIS Prime"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="Update JARVIS Prime",
            detail="This will exit the current session and run `jarvis-prime update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to JARVIS Prime! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to JARVIS Prime! Type your message or /help for commands."
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "JARVIS Prime")
...
def main(
    ...
    """
    JARVIS Prime CLI - Interactive AI Assistant
