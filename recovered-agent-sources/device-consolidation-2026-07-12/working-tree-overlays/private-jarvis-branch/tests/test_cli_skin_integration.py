...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_uses_jarvis_prime_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "JARVIS Prime v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "JARVIS PRIME" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_default_brand(self):
        ...
        assert "JARVIS PRIME" not in banner
