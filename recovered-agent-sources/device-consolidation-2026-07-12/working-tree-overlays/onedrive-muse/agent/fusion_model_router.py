...
class MoEModelRouter:
    ...
    @staticmethod
    def _infer_specialization_score(model: str, query_type: QueryType) -> float:
        """Infer a model suitability score from provider/model family hints.

        This keeps large pools such as all NVIDIA NIM models useful even when
        individual entries are not hand-authored in MODEL_SPECIALIZATIONS.
        """
        m = (model or "").lower()

        # Non-chat / guardrail / embedding / parsing models are useful tools,
        # but poor general reference experts for answer fusion.
        if any(k in m for k in (
            "embed", "retriever", "parse", "clip", "detector", "riva",
            "translate", "guard", "safety", "gliner", "deplot",
        )):
            return 0.25 if query_type != QueryType.VISION else 0.45

        # Coding specialists.
        if any(k in m for k in ("coder", "code", "codestral", "starcoder", "granite-34b-code", "deepseek-coder")):
            return {
                QueryType.CODE: 0.94,
                QueryType.MATH: 0.78,
                QueryType.ANALYTICAL: 0.78,
                QueryType.FACTUAL: 0.70,
                QueryType.CREATIVE: 0.55,
                QueryType.MULTILINGUAL: 0.60,
                QueryType.VISION: 0.25,
                QueryType.GENERAL: 0.80,
            }.get(query_type, 0.75)

        # NVIDIA Nemotron reasoning tiers.
        if "nemotron-3-ultra" in m or "nemotron-4-340b" in m or "nemotron-ultra" in m:
            return {
                QueryType.CODE: 0.86,
                QueryType.MATH: 0.92,
                QueryType.ANALYTICAL: 0.95,
                QueryType.FACTUAL: 0.88,
                QueryType.CREATIVE: 0.78,
                QueryType.MULTILINGUAL: 0.72,
                QueryType.VISION: 0.50,
                QueryType.GENERAL: 0.93,
            }.get(query_type, 0.88)
        if "nemotron-3-super" in m or "nemotron-super" in m or "llama-3.3-nemotron-super" in m:
            return {
                QueryType.CODE: 0.86,
                QueryType.MATH: 0.88,
                QueryType.ANALYTICAL: 0.92,
                QueryType.FACTUAL: 0.88,
                QueryType.CREATIVE: 0.78,
                QueryType.MULTILINGUAL: 0.72,
                QueryType.VISION: 0.45,
                QueryType.GENERAL: 0.91,
            }.get(query_type, 0.86)
        if "nemotron" in m and any(k in m for k in ("nano", "mini")):
            return {
                QueryType.CODE: 0.80,
                QueryType.MATH: 0.78,
                QueryType.ANALYTICAL: 0.82,
                QueryType.FACTUAL: 0.86,
                QueryType.CREATIVE: 0.68,
                QueryType.MULTILINGUAL: 0.66,
                QueryType.VISION: 0.62 if "vl" in m or "omni" in m else 0.35,
                QueryType.GENERAL: 0.82,
            }.get(query_type, 0.80)

        # Fast models and mixture-of-experts good for low latency diversity.
        if any(k in m for k in ("flash", "maverick", "minimax-m", "kimi", "step-3")):
            return {
                QueryType.CODE: 0.86 if "kimi" in m or "deepseek" in m else 0.78,
                QueryType.MATH: 0.78,
                QueryType.ANALYTICAL: 0.82,
                QueryType.FACTUAL: 0.88,
                QueryType.CREATIVE: 0.82,
                QueryType.MULTILINGUAL: 0.80,
                QueryType.VISION: 0.55 if "maverick" in m else 0.30,
                QueryType.GENERAL: 0.84,
            }.get(query_type, 0.80)

        # Large general reasoning families.
        if any(k in m for k in ("qwen", "deepseek", "mistral-large", "mistral-medium", "llama-3.1-70b", "llama3-chatqa")):
            return {
                QueryType.CODE: 0.84,
                QueryType.MATH: 0.88,
                QueryType.ANALYTICAL: 0.88,
                QueryType.FACTUAL: 0.84,
                QueryType.CREATIVE: 0.78,
                QueryType.MULTILINGUAL: 0.82 if "qwen" in m else 0.72,
                QueryType.VISION: 0.38,
                QueryType.GENERAL: 0.86,
            }.get(query_type, 0.82)

        # Vision-capable models.
        if any(k in m for k in ("vision", "vl", "omni", "vila", "neva", "fuyu")):
            return 0.88 if query_type == QueryType.VISION else 0.68
    ...
    def score_models(
        ...
        for model in self.available_models:
            ...
            score = specs.get(query_type)
            if score is None:
                score = self._infer_specialization_score(model, query_type)
            scores[model] = score
        return scores
