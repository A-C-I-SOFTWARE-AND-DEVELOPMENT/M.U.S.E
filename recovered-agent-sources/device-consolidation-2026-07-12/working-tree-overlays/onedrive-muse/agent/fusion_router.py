...
async def fuse_response(
    ...
    if cfg.get("difficulty_aware", True) and original_response:
        ...
        # Preserve the chosen depth/top_k so MoE can route over a large model
        # pool (for example all NVIDIA NIM models) without calling every model.
        cfg = depth_to_config(assessment, cfg)
        cfg["_fusion_depth"] = assessment.depth.name.lower()
        cfg["_fusion_top_k"] = len(cfg.get("reference_models", []))
        logger.info(
    ...
    if cfg.get("moe_routing", True):
        ...
        configured_models = list(cfg.get("reference_models", FUSION_CONFIG["reference_models"]) or [])
        # A large routing pool lets Hermes score all available NVIDIA NIM
        # models but only fan out to a small top-K for speed and rate-limit
        # safety. If omitted, the configured reference list is both the pool
        # and the selected list.
        all_models = list(cfg.get("model_pool") or configured_models)
        aggregator = cfg.get("aggregator_model", FUSION_CONFIG["aggregator_model"])

        # Create router with current model pool. The aggregator remains the
        # shared expert/anchor when it is also present in the pool.
        router = MoEModelRouter(
            available_models=all_models,
            anchor_model=aggregator,
        )

        # Route query to best models (like top-K expert selection). Respect
        # any ACT-selected top_k, otherwise use config.moe_top_k, otherwise
        # the size of the configured reference set. Clamp to the pool size.
        requested_top_k = cfg.get("_fusion_top_k", cfg.get("moe_top_k", len(configured_models) or len(all_models)))
        try:
            n_models = int(requested_top_k)
        except (TypeError, ValueError):
            n_models = len(configured_models) or len(all_models)
        max_top_k = int(cfg.get("moe_max_top_k", 8) or 8)
        n_models = max(1, min(n_models, max_top_k, len(all_models)))
        ...
        cfg["_moe_pool_size"] = len(all_models)
        cfg["_moe_top_k"] = n_models

        logger.info(
            "MoE routing: type=%s top_k=%d pool=%d models=%s",
            query_type.value, n_models, len(all_models), selected_models,
        )
