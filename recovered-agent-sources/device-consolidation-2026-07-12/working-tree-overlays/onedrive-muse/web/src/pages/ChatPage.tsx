...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    ...
    // Expose terminal instance for MuseChatOverlay navigation triggers
    (window as unknown as Record<string, unknown>).__hermesTerminal = term;
    ...
    ws.onmessage = (ev) => {
      ...
      if (chunk.includes(" ready ") || chunk.includes("─ ready") || chunk.includes("❯")) {
        setChatReady(true);
      }
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          ...
          {!chatReady && (
            <div className="pointer-events-none absolute left-4 top-4 z-20 rounded-full border border-cyan-300/30 bg-black/45 px-3 py-1.5 text-[0.68rem] uppercase tracking-[0.16em] text-cyan-100 shadow-lg backdrop-blur-md">
              Starting Hermes chat… first boot can take 20–60s
            </div>
          )}

          {/* MUSE identity + control overlay */}
          {isActive && (
            <MuseChatOverlay hostRef={hostRef} wsRef={wsRef} />
          )}
