...
// Lazy-load pages for code splitting (ChatPage stays eager for persistent terminal)
...
const MuseOmniPage = lazy(() => import("@/pages/MuseOmniPage"));
...
const BUILTIN_ROUTES_CORE: Record<string, ComponentType> = {
  ...
  "/musehq": () => (
    <FramePage
      title="musehq.io"
      subtitle="OpenCode-style MUSE cockpit shell with streaming chat UI, Atlas, Studio, and Observatory links."
      src="/musehq/"
    />
  ),
  "/nexus": () => (
    <FramePage
      title="NEXUS"
      subtitle="Unified M.U.S.E. command console: steering, agents, forge, fleet, models, observatory, and repo controls."
      src="/nexus/"
    />
  ),
  "/studio": () => <StudioShell />,
...
const BUILTIN_NAV_REST: NavItem[] = [
  { path: "/studio", label: "Studio", icon: Code },
  {
    path: "/sessions",
    labelKey: "sessions",
    label: "Sessions",
...
export default function App() {
  ...
  const isStudioRoute = normalizedPath === "/studio";
  const isOmniRoute = normalizedPath === "/omni";
  ...
  // ── Omni full-screen mode ───────────────────────────────────────
  // Make the command center the main thing: no nested dashboard chrome,
  // no double sidebars, just the cinematic operating surface.
  if (isOmniRoute) {
    return (
      <ErrorBoundary>
        <Suspense fallback={<LoadingSkeleton variant="card" className="h-dvh w-dvw" />}>
          <MuseOmniPage />
        </Suspense>
      </ErrorBoundary>
    );
  ...
  // When the user is on /studio, render the full-screen Studio shell
  // that acts as the container for everything, bypassing the dashboard
  // sidebar + chrome. The StudioShell has its own floating dock that
  // provides navigation to all dashboard tools.
  if (isStudioRoute) {
    return (
      <ErrorBoundary>
        <StudioShell />
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  ...
                  className="flex flex-col border-t border-current/10 pb-1"
                  role="group"
                >
                  <span
                    className={cn(
            ...
            >
              <div className="flex min-w-0 items-center gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-md border border-current/15 px-2 py-1 text-[0.6rem] text-midground/50 transition-colors hover:border-current/30 hover:text-midground/80"
      ...
      <NavLink
        ...
        className={({ isActive }) =>
          cn(
            ...
            "px-4 py-1",
            "font-mondwest text-[0.72rem] tracking-[0.11em]",
