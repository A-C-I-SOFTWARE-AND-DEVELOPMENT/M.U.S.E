...
@app.get("/api/muse/repo")
async def get_muse_repo(request: Request):
    """Live metadata for the local M.U.S.E checkout powering the dashboard.

    This endpoint intentionally reads only local repository metadata and small
    manifest/config files. It never returns secrets or file contents beyond
    high-level package/script names.
    """
    _require_token(request)

    # The entire body below does synchronous I/O (subprocess.run, os.walk,
    # file reads).  Running it directly in the async event loop blocks all
    # other HTTP/WebSocket handlers — including /api/pty chat sessions —
    # for up to 10+ seconds while git and os.walk traverse the repo.
    # Offload to a thread so the event loop stays responsive.
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, _get_muse_repo_sync)


def _get_muse_repo_sync() -> dict:
    """Synchronous repo metadata reader — called from a thread executor."""

    root = Path(os.environ.get("MUSE_REPO_ROOT") or PROJECT_ROOT).resolve()

    def _run_git(args: List[str], timeout: float = 3.0) -> str:
        try:
            proc = subprocess.run(
                ["git", *args],
                cwd=str(root),
    ...
    def _count_files(base: Path, glob: str) -> int:
        excluded = {
            ".git", "node_modules", ".next", "dist", "build", "web_dist",
            "__pycache__", ".venv", "venv", ".turbo", ".cache",
            "coverage", ".pytest_cache", ".mypy_cache", ".ruff_cache",
        }
        try:
            count = 0
            suffix = glob[1:] if glob.startswith("*") else ""
            if not base.exists():
                return 0
    ...
    web_package = _safe_json(root / "web" / "package.json")
    nexus_package = _safe_json(root / "apps" / "nexus" / "package.json")

    branch = _run_git(["rev-parse", "--abbrev-ref", "HEAD"])
    commit = _run_git(["rev-parse", "--short", "HEAD"])
    commit_subject = _run_git(["log", "-1", "--pretty=%s"])
    status_lines = _run_git(["status", "--short"], timeout=5.0).splitlines()
    changed = [line for line in status_lines if line.strip()]
    ...
    return {
        ...
        "packages": {
            ...
            "web": {
                "name": web_package.get("name"),
                "version": web_package.get("version"),
                "scripts": sorted((web_package.get("scripts") or {}).keys()),
                "dependencies": len(web_package.get("dependencies") or {}),
                "dev_dependencies": len(web_package.get("devDependencies") or {}),
            },
            "nexus": {
                "name": nexus_package.get("name"),
                "version": nexus_package.get("version"),
                "scripts": sorted((nexus_package.get("scripts") or {}).keys()),
                "dependencies": len(nexus_package.get("dependencies") or {}),
                "dev_dependencies": len(nexus_package.get("devDependencies") or {}),
            },
        },
        "counts": {
            "python": _count_files(root, "*.py"),
            "typescript": _count_files(root, "*.ts") + _count_files(root, "*.tsx"),
            "javascript": _count_files(root, "*.js") + _count_files(root, "*.jsx"),
            "markdown": _count_files(root, "*.md"),
            "skills": _count_files(root / "skills", "SKILL.md"),
            "tests": _count_files(root / "tests", "test_*.py"),
        },
        "surfaces": {
            "web": (root / "web").exists(),
            "nexus": (root / "apps" / "nexus").exists(),
            "musehq": (root / "web" / "musehq").exists(),
            "studio": (root / "agent" / "studio").exists(),
            "dashboard_dist": WEB_DIST.exists(),
        },
    }
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    try:
        while True:
            ...
            await loop.run_in_executor(None, bridge.write, raw)
...
def mount_spa(application: FastAPI):
    ...
    def _serve_static_index(index_path: Path, prefix: str = ""):
        """Serve a nested static app index with dashboard globals injected.

        This lets packaged sub-apps such as /musehq/ and /nexus/ call the same
        protected dashboard APIs without an unauthenticated token endpoint.
        """
    ...
    async def serve_spa(full_path: str, request: Request):
        ...
        if full_path and file_path.resolve().is_relative_to(WEB_DIST.resolve()) and file_path.is_dir():
            nested_index = file_path / "index.html"
            if nested_index.is_file():
                return _serve_static_index(nested_index, prefix)
        ...
        if (
            ...
            if file_path.name in ("index.html", "omni.html"):
                return _serve_static_index(file_path, prefix)
...
async def get_dashboard_themes():
    ...
    """
    config = load_config()
    active = cfg_get(config, "dashboard", "theme", default="muse")
