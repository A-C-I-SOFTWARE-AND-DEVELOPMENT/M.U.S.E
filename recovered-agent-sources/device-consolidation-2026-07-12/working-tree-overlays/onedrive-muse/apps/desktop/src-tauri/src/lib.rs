...
const CREDENTIAL_SERVICE: &str = "com.aci.muse.desktop";
const CREDENTIAL_USER: &str = "local-gateway-device-token";

fn device_token_entry() -> Result<keyring::Entry, String> {
    keyring::Entry::new(CREDENTIAL_SERVICE, CREDENTIAL_USER).map_err(|error| error.to_string())
}

/// Read the per-device token from the operating-system credential vault. The
/// trusted bundled UI keeps the returned value in memory, never localStorage.
#[tauri::command]
fn credential_read() -> Result<Option<String>, String> {
    match device_token_entry()?.get_password() {
...
#[tauri::command]
fn credential_store(token: String) -> Result<(), String> {
...
fn credential_delete() -> Result<(), String> {
    match device_token_entry()?.delete_credential() {
        Ok(()) | Err(keyring::Error::NoEntry) => Ok(()),
        Err(error) => Err(error.to_string()),
    }
}

/// A simple HTTP proxy command
