...
function PairCard({ onPaired }: { onPaired: () => void }) {
  ...
  if (autoPairing) {
    return (
      <div className="card local-connect-card" role="status">
        <span className="connection-orbit" aria-hidden="true" />
        <div>
          <div className="eyebrow">Secure local connection</div>
          <strong>Connecting this PC automatically</strong>
          <p className="muted">
            Muse is creating a device-specific connection to the gateway on this
            PC. No code or owner phrase is required.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="card" style={{ borderColor: "var(--ring-1)" }}>
      <SectionHeader
        ...
        title="Owner-gated remote pairing"
        trailing={<span className="pill">remote connection</span>}
      />
      <p className="muted">
        This gateway is remote or externally reachable. Generate a short-lived
        pairing code, then confirm it with the owner phrase.
      </p>
      <div className="row">
        <input
          type="text"
