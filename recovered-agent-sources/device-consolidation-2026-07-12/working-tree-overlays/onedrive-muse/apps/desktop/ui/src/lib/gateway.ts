...
/**
 * Hydrate the native-shell credential from the OS vault before first paint.
 * A legacy localStorage token is migrated once, then erased. Browser builds
 * retain localStorage because they don't have an operating-system vault.
 */
export async function initializeCredentialStore(): Promise<void> {
  if (!isTauri()) return;
  try {
    const stored = await tauriInvoke("credential_read");
    nativeTokenCache = typeof stored === "string" ? stored.trim() : "";
    const legacy = safeLocalStorageGet(TOKEN_KEY).trim();
    if (!nativeTokenCache && legacy) {
      await tauriInvoke("credential_store", { token: legacy });
      nativeTokenCache = legacy;
    }
    safeLocalStorageRemove(TOKEN_KEY);
  } catch {
    // Vault unavailable: keep a legacy token in memory for this session only.
    nativeTokenCache = safeLocalStorageGet(TOKEN_KEY).trim();
    safeLocalStorageRemove(TOKEN_KEY);
  }
}

export function getToken(): string {
  return isTauri() ? nativeTokenCache : safeLocalStorageGet(TOKEN_KEY) || "";
}

export function setToken(token: string): void {
  const value = token.trim();
  if (isTauri()) {
    nativeTokenCache = value;
    safeLocalStorageRemove(TOKEN_KEY);
    const operation = value ? tauriInvoke("credential_store", { token: value }) : tauriInvoke("credential_delete");
    void operation.catch(() => {});
  } else {
    safeLocalStorageSet(TOKEN_KEY, value);
  }
  try {
