  ...
  return (
    <div className="card" style={{ borderColor: "var(--ring-1)" }}>
      <div className="row">
        <b>Device pairing</b>
        <span className="grow" />
        <span className="pill">{paired ? "securely connected ✓" : "not connected"}</span>
        <span className="pill">{isLoopbackBase() ? "OS-protected" : "owner-gated"}</span>
      </div>
      {isLoopbackBase() && (
        <>
          <p className="muted">
            On this PC, muse connects to its local gateway <b>automatically</b> —
            no code or owner phrase needed. Its device credential is protected by
            your operating system. Use the button below to reconnect (for example
            after disconnecting this device).
          </p>
          <div className="row">
      ...
      {!isLoopbackBase() && (
        ...
        stored in this device's operating-system credential vault. The owner phrase is never
        stored.
      </p>
