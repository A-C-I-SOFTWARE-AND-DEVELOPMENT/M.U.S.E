...
import { CommandPalette, NavIcon } from "./components/CommandPalette";
...
export function App() {
  ...
  const [activeId, setActiveId] = useState<string>(() => {
    ...
    return routeList.find((route) => route.id === "chat")?.id ?? routeList[0]?.id ?? "";
  });
    ...
    <>
      ...
      <div className="app">
        ...
        <header className="app-header">
          ...
          <CommandPalette routes={routeList} onSelect={select} />
        ...
        <nav className="app-nav" aria-label="Muse destinations">
          ...
          <div className="nav-stack">
            {routeList.filter((route) => route.id !== "settings").map((r) => (
              <button
                key={r.id}
          ...
          {routeList.filter((route) => route.id === "settings").map((r) => (
            <button
              key={r.id}
              className={"nav-item nav-settings" + (r.id === active?.id ? " active" : "")}
              onClick={() => select(r.id)}
              aria-current={r.id === active?.id ? "page" : undefined}
              title={r.label}
            >
          ...
          <div className="nav-footer">
            <span className={"dot " + (health === "online" ? "ok" : health === "offline" ? "off" : "")} />
            <span>{health === "online" ? "Local brain ready" : health === "offline" ? "Brain offline" : "Starting Muse"}</span>
          </div>
        </nav>
