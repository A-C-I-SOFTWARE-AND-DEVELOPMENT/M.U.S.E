...
def normalize_path(path: str) -> str:
    ...
    """
    # ``expanduser`` ignores HOME on Windows in favor of USERPROFILE, which
    # makes an explicitly configured workspace home ineffective and breaks
    # hermetic runners.  Honour HOME for the ordinary current-user ``~`` form
    # on every platform; leave ``~other`` to the platform implementation.
    if path == "~" or path.startswith(("~/", "~\\")):
        configured_home = os.environ.get("HOME")
        if configured_home:
            path = configured_home + path[1:]
