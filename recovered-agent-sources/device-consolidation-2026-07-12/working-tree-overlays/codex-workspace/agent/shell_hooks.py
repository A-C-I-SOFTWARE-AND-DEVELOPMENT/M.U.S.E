...
def _spawn(spec: ShellHookSpec, stdin_json: str) -> Dict[str, Any]:
    ...
    # Windows cannot execute POSIX shebang scripts directly even when Git Bash
    # is installed. Route explicit shell scripts through bash while preserving
    # argv boundaries and shell=False; native executables remain unchanged.
    if os.name == "nt" and Path(argv[0]).suffix.lower() in {".sh", ".bash"}:
        script_path = Path(argv[0])
        if script_path.drive:
            # Git Bash expects MSYS drive syntax when launched from a native
            # Windows process; ``C:/...`` is not resolved as a script path.
            drive = script_path.drive.rstrip(":").lower()
            rest = script_path.as_posix()[len(script_path.drive):]
            argv[0] = f"/{drive}{rest}"
        else:
            argv[0] = script_path.as_posix()
        argv.insert(0, shutil.which("bash") or "bash")
    ...
    try:
        ...
        # Windows can sporadically return STATUS_DLL_INIT_FAILED when Git
        # Bash starts after a long, process-heavy agent/test session. Retry the
        # launch once; ordinary hook exit codes (including 1) are never retried.
        if os.name == "nt" and proc.returncode in {0xC0000142, -1073741502}:
            logger.warning(
                "shell hook interpreter initialization failed; retrying once "
                "(event=%s command=%s)",
                spec.event,
                spec.command,
            )
