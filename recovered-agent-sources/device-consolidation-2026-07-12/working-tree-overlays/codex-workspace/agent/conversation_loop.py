...
# Earlier assistant tool calls are replayed in every subsequent API request.
# Their arguments are immutable after creation, so repeatedly parsing and
# serializing the same JSON once per agent iteration is needless O(T * I)
# work (T historical tool calls, I API iterations).  Keep a bounded cache for
# normal-sized payloads; large payloads bypass it rather than pinning memory.
_TOOL_ARGUMENT_CACHE_MAX_CHARS = 16_384


def _canonicalize_tool_call_arguments_uncached(arguments: str, tool_name: str) -> str:
    """Return deterministic JSON, falling back to the established repair path."""
    try:
        args_obj = json.loads(arguments)
        return json.dumps(args_obj, separators=(",", ":"), sort_keys=True)
    except Exception:
        return _repair_tool_call_arguments(arguments, tool_name)


@lru_cache(maxsize=2048)
def _canonicalize_tool_call_arguments_cached(arguments: str, tool_name: str) -> str:
    """Bounded memoization for immutable, replayed tool-call arguments."""
    return _canonicalize_tool_call_arguments_uncached(arguments, tool_name)


def _canonicalize_tool_call_arguments(arguments: str, tool_name: str) -> str:
    """Canonicalize tool arguments without retaining unusually large payloads."""
    if isinstance(arguments, str) and len(arguments) <= _TOOL_ARGUMENT_CACHE_MAX_CHARS:
        return _canonicalize_tool_call_arguments_cached(arguments, tool_name)
    return _canonicalize_tool_call_arguments_uncached(arguments, tool_name)


def _emit_post_api_request(**kwargs: Any) -> None:
    """Emit one best-effort outcome event for every provider API attempt."""
    try:
        from hermes_cli.plugins import invoke_hook
        invoke_hook("post_api_request", **kwargs)
    except Exception:
        logger.debug("post_api_request hook failed", exc_info=True)
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Record latency per provider attempt, not per retry cycle.
                # Keeping this timestamp outside ``while retry_count`` made
                # post_api_request latency include earlier failures and
                # backoff, which corrupts provider health/benchmark metrics.
                ...
                if response_invalid:
                    ...
                    _emit_post_api_request(
                        ...
                        api_success=False,
                        error=agent._clean_error_message(
                ...
                if agent.thinking_callback:
                    agent.thinking_callback("")
                # An interrupt can be raised by a pre-transport guard.  The
                # attempt has no latency in that case, but its observer event
                # must still be emitted instead of masking the interruption.
                api_elapsed = (
                    time.time() - api_start_time
                    if api_start_time is not None else None
                )
                _emit_post_api_request(
                    task_id=effective_task_id,
                    session_id=agent.session_id or "",
                ...
                _emit_post_api_request(
                    ...
                    api_mode=agent.api_mode,
                    api_call_count=api_call_count,
                    api_duration=_api_error_duration,
                    message_count=len(api_messages),
                    response_model=None,
                    response=None,
                    usage=None,
    ...
    # Plugin hook: post_turn
    #
    # Fire after the result has been assembled so quality/telemetry plugins
    # can observe the terminal state of *every* turn, including interrupted,
    # budget-exhausted, and empty-response recovery paths.  It is deliberately
    # observer-only: hook return values are ignored and plugin failures cannot
    # affect the response or the conversation loop.
    try:
        from hermes_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "post_turn",
        ...
        logger.warning("post_turn hook failed: %s", exc)
