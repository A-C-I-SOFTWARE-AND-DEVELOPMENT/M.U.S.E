...
class TestAddRotatingHandler:
    ...
    def test_rollover_permission_error_reopens_stream(self, tmp_path):
        """A cross-process Windows lock must not make logging raise."""
        ...
        try:
            with (
                patch.object(RotatingFileHandler, "doRollover", side_effect=PermissionError),
                patch.object(handler, "_open", return_value=replacement) as reopen,
            ):
                handler.doRollover()
    ...
    @pytest.mark.skipif(
        os.name == "nt",
        reason="Windows ACLs do not expose POSIX group mode bits through stat()",
    )
