...
def _bash_path(path: Path) -> str:
    """Render a native test path for the Git Bash subprocess."""
    if os.name == "nt" and path.drive:
        drive = path.drive.rstrip(":").lower()
        return f"/{drive}{path.as_posix()[len(path.drive):]}"
...
class TestCallbackSubprocess:
    ...
    @pytest.mark.skipif(os.name != "nt", reason="Windows-specific process status")
    def test_retries_transient_dll_initialization_failure(self, monkeypatch):
        attempts = [
            subprocess.CompletedProcess([], 0xC0000142, "", ""),
            subprocess.CompletedProcess([], 0, "{}\n", ""),
        ]
        monkeypatch.setattr(shell_hooks.subprocess, "run", lambda *a, **kw: attempts.pop(0))

        result = shell_hooks._spawn(
            shell_hooks.ShellHookSpec(event="pre_tool_call", command="cmd.exe /c exit 0"),
            "{}",
        )
...
class TestAllowlistConcurrency:
    ...
    @pytest.mark.skipif(
        os.name == "nt",
        reason="Windows ACLs do not expose POSIX executable mode bits",
    )
