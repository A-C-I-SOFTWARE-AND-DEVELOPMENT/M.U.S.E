# muse Local Orchestrator (Android)

muse is a **private personal orchestration tool**. It runs on a single
...
- **Not** a credential broker. muse does not scrape cookies, extract
...
HermesService.onStartCommand
    │  ├─ Log: "muse local orchestrator started"
...
muse does not call OpenAI, Anthropic, or any other provider directly
...
This means muse piggy-backs on the user's existing subscriptions
...
- muse cannot silently consume ChatGPT / Claude subscriptions like an
  API. Every handoff is a user-driven copy/paste or deep link.
- muse cannot drive Claude Code unless using its official allowed
...
The Android app is the **cockpit**. The muse backend (CLI + gateway
...
2. **Pipeline handoff** — the cockpit hits the backend gateway with a
   ...
   - `ai-improvement-radar` and the competitive feature harvester at
     ...
     `self-improvement-loop`, which can propose patches to muse
...
nuke the app from orbit, use Android's system Settings → Apps → muse
