...
Hermes can talk to GitHub two ways:

1. **Native plugin** — `github_assistant`, ships in this repo at
   [`plugins/github_assistant/`](../plugins/github_assistant/). Eight
   first-class Hermes tools, three explicit safety gates, talks straight
   to the GitHub REST API.
2. **MCP server** — point Hermes at the official
...
4. **Repository access**: pick *Only select repositories* and choose
   the exact repos you want Hermes to touch. Never grant *All
   repositories* unless you're auditing your full personal account from
   one Hermes session and you know what you're doing.
5. **Repository permissions** — set only what you need:
   ...
   as for Hermes' `enterprise.secrets` ACL: the token should only be
   able to do what the agent might legitimately do on your behalf.

6. **Account permissions**: leave at defaults (no access). Hermes
   doesn't need them.
7. Generate, copy the `github_pat_*` value once, and put it in your
   Hermes env (next section).
...
Hermes loads `.env` before any tool call. Don't paste the token into
...
Restart Hermes (or `/reload` if your CLI supports it). `hermes tools
...
is the difference between "Hermes can read GitHub" and "Hermes can
...
The `${GITHUB_PERSONAL_ACCESS_TOKEN}` syntax is expanded by Hermes' own
config loader (`hermes_cli.config._expand_env_vars`); the literal
string never reaches disk. If the env var isn't set when Hermes
...
If you do nothing, Hermes does not have GitHub access. If you set the
token only, Hermes still does not have GitHub access (the master
...
restarted Hermes after editing it.
