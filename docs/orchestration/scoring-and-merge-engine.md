...
muse orchestrates several workers in parallel against the same task.
...
| `local_first_fit` | Penalises external-network references in the patch (muse is local-first). |
