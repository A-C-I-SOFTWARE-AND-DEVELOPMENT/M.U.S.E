...
> next layer of muse orchestration. Nothing here is wired into the
...
muse already has the right *primitives* for orchestration:
...
- The Android local orchestrator (`docs/hermes-local-orchestrator.md`)
  defines a hand-off model where muse prepares a structured prompt
...
bespoke batch script. Phase 7 turns that workflow into a muse
