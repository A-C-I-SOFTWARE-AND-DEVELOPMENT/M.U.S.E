...
Hermes, workers are **profiles** — named processes with their own
...
`hermes profile list` shows what's loaded. `hermes profile create
...
will adapt to whatever it finds via `hermes profile list`. It does
not invent names.
...
backend that owns its shell, filesystem, and network. Hermes ships
seven:
...
tooling can read it without a Hermes runtime.
