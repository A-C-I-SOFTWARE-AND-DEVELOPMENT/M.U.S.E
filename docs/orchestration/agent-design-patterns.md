# Composable Agent Design Patterns (in JARVIS terms)

> **Status:** Guidance doc, 2026-06-04. JARVIS already *has* the primitives for
...
composable patterns, not heavyweight frameworks.** Hermes/JARVIS independently
...
## The five workflow patterns → JARVIS primitives

| Pattern | What it is | Where it already lives in JARVIS |
|---|---|---|
...
| **Orchestrator-workers** | A lead LLM dynamically splits work, delegates, synthesizes | JARVIS Prime runtime ([`runtime.py`](../../hermes_cli/jarvis_prime/runtime.py)) over worker profiles ([`worker_registry.py`](../../hermes_cli/jarvis_prime/worker_registry.py)); the `delegate_task` tool |
...
that explore in parallel, then synthesizes. JARVIS's analogue:

- **Lead** = JARVIS Prime runtime / the AOS Council Director.
...
This is the pattern JARVIS most under-uses, and the one the **self-audit harness**
...
- **Generator** — the builder worker (or JARVIS itself).
- **Evaluator** — a *separate* grader with an explicit rubric. In JARVIS the
  rubric is the [JARVIS Constitution](../jarvis-constitution.md); the graders are
...
an evaluator-optimizer applied to JARVIS *itself*: an auditor generates risky
...
auditor→target→judge loop expressed in JARVIS's own primitives.
...
| Split an open-ended goal | Orchestrator-workers | JARVIS runtime + `delegate_task` |
| Raise output quality / catch risk | Evaluator-optimizer | builder≠reviewer + Constitution rubric |
| Audit JARVIS's own behavior | Evaluator-optimizer (self) | the `self_audit/` harness |
