# muse Skill Map — AoS Council

This doc maps every AoS (Architecture of Solutions) council role to its
muse skill on disk, its slash command, and its purpose. It is the
canonical index for the Phase 03 conversion that brought the council
into muse as native skills (no `.claude/agents/` dependency).

muse scans skills under `~/.hermes/skills/` (seeded from
...
| Claude agent (upstream) | muse skill path | Slash command | Purpose |
|---|---|---|---|
...
| `codex-dispatch-governor` | `skills/codex-dispatch-governor/SKILL.md` | `/codex-dispatch-governor` | Builds handoff packets for in-muse subagents, external coding agents (Codex/Claude Code/Cursor), or the Android manual workflow. |
...
The council only references tools that muse ships natively. No
...
- Upstream documents (any `.claude/agents/` source you may merge in
  ...
  `contrarian-red-flag-analyst`. The canonical muse name is
