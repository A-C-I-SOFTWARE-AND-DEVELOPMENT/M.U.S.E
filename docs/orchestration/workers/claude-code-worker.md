...
or a one-off script) whenever Hermes wants Claude Code's opinion on
...
- Workspace is materialised, but Hermes will never invoke the CLI.
...
- Caller is opting into having Hermes shell out to the local CLI.
