...
The Goose worker is the muse adapter for
[Block/Goose](https://block.github.io/goose/), an open-source local
agent runtime with a recipe + extension model. Like every muse
worker, the adapter only invokes the user's local `goose` CLI — muse
...
The user reviews, runs, and stays in control. **muse never invokes
...
- muse does not install Goose or any extensions for the user.
