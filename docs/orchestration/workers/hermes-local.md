...
The Hermes Local worker is the **always-available** adapter that
...
and runs anywhere Hermes runs — Linux, macOS, Windows, Termux — with
...
Hermes Local is the first worker every orchestrated job invokes. It:
...
read its output to ground their prompts. Without Hermes Local the
...
Hermes Local infers commands from the following sources:
...
Hermes Local is an evidence worker, not a judgement worker — it does
