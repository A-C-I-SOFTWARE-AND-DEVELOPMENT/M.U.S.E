...
| MUSE-branded TUI is the default look | `hermes_cli/skin_engine.py` — runtime default skin is `singularity` (`agent_name="M.U.S.E."`, MUSE welcome/response label/banner) |
...
| Desktop app one-click | `apps/desktop/`: Tauri v2 + React 19; `cargo tauri build` produces installers; MUSE bundle id `com.aci.muse` / "M.U.S.E." window; one-button CI release lane (`muse-desktop-release.yml`, signed or clearly-unsigned); shell auto-starts the backend via `muse cockpit serve` |
...
2. **Terminal branding sweep** (`cli.py`, `hermes_cli/main.py`,
   ...
   history labels `◆ M.U.S.E.:`, status panel, busy-mode prose, clarify
   panel, response-label fallbacks `◉ M.U.S.E.`.
3. **Agent identity** (`agent/prompt_builder.py`, `hermes_cli/default_soul.py`)
   ...
   M.U.S.E. (Multi-Use Synaptic Entity), an intelligent AI operating
...
4. **Gateway + platform defaults** — Telegram topic copy, gateway hints,
   ...
   `◉ *M.U.S.E.*`, Home Assistant notification title, Matrix device name,
...
7. **Setup tooling** — `setup-hermes.sh` shows the M.U.S.E. banner, symlinks
...
8. **Docs/metadata** — `web/README.md`, `SETUP.md` (title, repo slug
   `A-C-I-SOFTWARE-AND-DEVELOPMENT/M.U.S.E`, clone paths), `flake.nix`
   description.
