...
*after* the image is built (`muse profile create coder` →
...
- Dynamic registration is limited to per-profile gateways — one service per
  ...
  profile so `muse gateway start` (no `-p`) has somewhere to land.
...
4. Optionally backgrounded `muse dashboard` in a subshell when
...
`muse gateway start`, `coder gateway start` (profile alias), and
`muse -p <profile> gateway start` all spawn a gateway process scoped to a
given profile. See
...
| `chat -q "..."` | `muse chat -q "..."`, container exits with hermes exit code |
...
(`muse -p <profile> gateway run`, or `muse gateway run` for the default
...
When `muse profile create <name>` runs inside the container, the
profile-creation code path calls
`ServiceManager.register_profile_gateway(<name>)` if
`supports_runtime_registration()` is True. When `muse profile delete
...
Existing per-profile `muse -p <profile> gateway start/stop/restart` CLI
...
`muse gateway start` (no `-p`) targets a special `gateway-default` slot
...
`--all` lifecycle (`muse gateway stop --all`, `... restart --all`)
...
| A `running` gateway that's actually broken auto-restarts into a crash loop after every container restart | Low | Medium | s6 `finish` script `max_restarts` cap (planned); follow-up: `muse doctor` alerts when N consecutive container restarts ended in `startup_failed` |
...
- **`gateway-default` slot:** always registered by the reconciler so
  `muse gateway start` (no `-p`) has somewhere to land.
...
- [x] `muse profile create test` inside a container creates
      `/run/service/gateway-test/`
- [x] `muse -p test gateway start` inside a container dispatches through s6
- [x] `muse -p test gateway stop` inside a container cleanly stops via s6
- [x] `muse profile delete test` inside a container removes
      `/run/service/gateway-test/`
- [x] Profile gateway logs persist at
      `$HERMES_HOME/logs/gateways/test/current`
- [x] `muse status` inside the container shows `Manager: s6`
- [x] `muse gateway start` (no `-p`) inside a container targets
      `gateway-default` and runs against the root profile
- [x] `muse gateway stop --all` / `... restart --all` iterate every
...
- [x] `muse gateway install/start/stop` on Linux host and macOS host
