# Hermes private/local security — in plain English

Hermes can run entirely on your own machine, without sending a byte to
...
1. **Your laptop is on the internet.** Browser tabs, email clients,
   ...
   process that ever runs as you. Hermes does not stop that — but it
   stops *Hermes itself* from making the leak worse, and it makes the
...
3. **The agent is connected to high-impact systems.** Hermes can
...
## What Hermes treats as a real boundary
...
For real containment, run Hermes inside a sandbox (Docker, OpenShell,
firejail). The private/local mode plays nicely with all of them.

## What "private/local" mode actually changes

When you flip Hermes into private/local mode (see
...
| Gateway adapters         | Telegram / Discord / Slack / WhatsApp / Signal / Email disabled. Android cockpit reaches Hermes over LAN or loopback only.        |
...
1. **Put your `.env` somewhere git can't reach it.** Hermes reads
...
5. **Run `git diff --cached | hermes` before pushing.** The CLI hook
   ...
   something, Hermes refuses to push and tells you which line.
...
| `~/.hermes/.env`                        | The one credential file Hermes reads by default.   |
...
| `skills/security-architect/SKILL.md`    | The skill Hermes loads when you ask it to *think* about security. |

## Further reading

- [`SECURITY.md`](../../SECURITY.md) — the trust model and reporting policy.
- [`docs/orchestration/private-local-mode.md`](../orchestration/private-local-mode.md)
  — how to take Hermes off the network.
