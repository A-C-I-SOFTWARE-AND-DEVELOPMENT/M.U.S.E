...
A short, practical guide to where muse reads secrets from, what
...
2. **`$HERMES_HOME/.env`** — same file, different path, when you've
   pointed muse at a custom home directory.
3. **Process environment** — anything already in `os.environ` when
   muse started. This is how CI, Docker, and OS-keychain helpers
   inject values.
...
5. **Android secure storage** — Android EncryptedSharedPreferences,
   accessed indirectly through the muse Android app.
...
## Rules — what muse will and won't do
...
3. Restart muse or run `/reload-skills` to pick the value up.
...
- **The agent's own RAM.** Anything resident in the muse Python
