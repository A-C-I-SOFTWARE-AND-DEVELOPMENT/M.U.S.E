...
A short, practical guide to where Hermes reads secrets from, what
...
## Where Hermes reads secrets from
...
2. **`$HERMES_HOME/.env`** — same file, different path, when you've
   pointed Hermes at a custom home directory.
3. **Process environment** — anything already in `os.environ` when
   Hermes started. This is how CI, Docker, and OS-keychain helpers
   inject values.
...
5. **Android secure storage** — Android EncryptedSharedPreferences,
   accessed indirectly through the Hermes Android app.
...
## Rules — what Hermes will and won't do
...
3. Restart Hermes or run `/reload-skills` to pick the value up.
4. Test that it loaded with: `hermes doctor` (the line "credentials
...
- **The agent's own RAM.** Anything resident in the Hermes Python
