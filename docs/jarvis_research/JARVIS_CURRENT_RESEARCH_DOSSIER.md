# JARVIS — Current Research Dossier

Purpose: ground JARVIS's safety, routing, and memory design in current
...
- LLM06 Excessive Agency: bounded work packets, builder/reviewer split,
  eight verification gates, no owner-gated execution by JARVIS.
...
- NIST AI Risk Management Framework (AI RMF 1.0) organizes risk work into
  **Govern, Map, Measure, Manage**. JARVIS's owner gates + audit ledger +
...
- Claude Code supports memory (CLAUDE.md), subagents, hooks, permissions,
  and skills. JARVIS adds `.claude/agents/jarvis-*` (read-only reviewers)
...
- Codex-style agents use background/eval/approval patterns. JARVIS treats
...
- vLLM (continuous batching), SGLang, Ollama, and llama.cpp expose
  OpenAI-compatible endpoints; JARVIS emits `local_endpoint_packet` with
