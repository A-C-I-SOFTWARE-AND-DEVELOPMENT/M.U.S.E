# Hermes phone-first runtime (Termux backend service)

This is the operator manual for running Hermes Agent as a real
...
`node`, `ssh`, `pkg`) inside the normal Android app sandbox. Hermes
...
6. **Hermes install state** — `HERMES_HOME`, `hermes-agent`
...
expose it (which Hermes does not do for you).
...
have nothing to do with Hermes itself — the OOM killer reclaims
