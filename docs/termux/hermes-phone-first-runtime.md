# muse phone-first runtime on Termux

muse runs as a real local development backend on an Android phone
...
`ssh`, `pkg`) inside a normal Android app sandbox. muse treats this as
...
6. **muse install state** — `HERMES_HOME`, `hermes-agent` checkout,
