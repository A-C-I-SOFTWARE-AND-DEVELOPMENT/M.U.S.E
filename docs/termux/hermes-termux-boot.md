# Autostarting Hermes with Termux:Boot

`hermes-termux-service.sh` is designed to be invoked by [Termux:Boot](https://wiki.termux.com/wiki/Termux:Boot)
so the Hermes runtime is up before you unlock the phone in the morning.
...
not already exist, then drop in a thin wrapper that calls the Hermes
service script:
...
# Wake the device long enough to bring Hermes up reliably.
termux-wake-lock
...
gateway is a separate, opt-in service that bridges Hermes to external
messaging platforms.
...
launch `hermes gateway run` after the API is up, using the same wake
...
To stop Hermes from coming up at boot:

```bash
rm ~/.termux/boot/10-hermes
```

The next reboot will skip Hermes entirely. The boot script is the only
