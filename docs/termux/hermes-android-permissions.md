# Android permissions and constraints for muse on Termux
...
out the constraints that matter for the muse phone-first runtime,
...
Termux itself is exempt from background restrictions. muse inherits
...
Termux's foreground notification is what gives muse a stable home
...
Termux has two distinct filesystems, and muse uses both:
...
`~/storage` symlinks. muse itself does not require shared storage —
...
| Shared storage access            | `termux-setup-storage` once; optional for muse itself |
