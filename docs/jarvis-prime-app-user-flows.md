# Jarvis Prime — Android App User Flows
...
>
> Twenty primary flows for the Jarvis Prime Android app. Each flow
...
- *Jarvis* is Jarvis Prime, the operating partner running in the
  gateway.
- *Gateway* is the Hermes Python core / AOS Council / Jarvis Prime
...
1. [Open app and see Jarvis status](#1-open-app-and-see-jarvis-status)
2. [Ask Jarvis a casual question](#2-ask-jarvis-a-casual-question)
...
## 1. Open app and see Jarvis status

**Trigger.** Owner taps the Jarvis Prime app icon, or unlocks the
phone with the Jarvis Prime widget pinned.
...
- **Emergency stop engaged from a prior session.** Icon enters
  *Stopped*; persistent banner *"Jarvis is stopped. Tap to
  resume."* across the app.

**Safety.** Home never auto-dispatches anything on open. No write
fires from open.

**Accessibility.** Status pill announces *"Jarvis: connected"*
...
## 2. Ask Jarvis a casual question
...
4. Jarvis classifies the request as Companion / Strategy and routes
...
- **Out-of-scope (touches an Owner Gate).** Jarvis classifies the
...
- **Rate limit / 429.** Banner *"Jarvis is rate-limited by the
...
characters. *Abort* reads as *"Stop Jarvis"*.

**Offline.** Compose works; the message goes to the outbox; banner
*"Offline — messages will send when Jarvis is back"*. The owner
...
- **Out-of-grammar in driving mode.** Jarvis speaks *"I only take a
  small set of commands while you're driving. Say 'Jarvis, end
...
even *"Jarvis, approve"* opens the relevant approval screen, never
fires the write. The exception is *"Jarvis, stop everything"* which
...
and Jarvis surfaces a *Convert to task* chip beneath the latest
assistant reply.
...
1. Owner taps *Convert to task*. **Task draft sheet** opens with:
   ...
   - **Clean task title** (Jarvis-suggested),
   - **Short summary** (Jarvis-generated),
...
- **Gateway 5xx on create.** Task draft is preserved; banner
  *"Couldn't reach Jarvis — kept as draft"*. Drafts live under
...
- **SSE drop.** *Live* pill turns amber: *"Live updates paused —
  ...
  renders.
- **Empty.** *"No tasks yet. Ask Jarvis to do something."* with a
...
- **Approval revoked between view and confirm.** Sheet shows
  *"This approval was withdrawn by Jarvis — nothing to approve."*
...
are dimmed with *"Approvals need Jarvis online — they live in the
...
- **Evidence card cannot be rendered.** Banner *"Jarvis could not
...
- **Impact Report missing a section.** That section renders as
  *"Jarvis could not produce this section."* Owner can still
...
rejection. Jarvis is allowed to propose again with new context.
...
Jarvis Prime activity immediately.

**Happy path.**

1. Owner **double-taps the interactive icon** (any screen) or
   long-presses the lock-screen widget, or speaks *"Jarvis, stop
   everything"* if voice mode is on.
2. **Emergency stop confirm sheet** rises immediately. The sheet
   shows: *"Stop Jarvis right now? All running tasks pause, all
   approvals are blocked, and Jarvis won't take any new action
   until you Resume."*
3. Owner taps **Stop now** (or speaks *"Stop now"*).
4. Local effect (≤ 200 ms): icon flips to *Stopped* (red bar)
   across the app; persistent banner *"Jarvis is stopped"* appears
...
- **Owner triggers stop on a screen that already shows Stopped.**
  No-op. Sheet shows *"Jarvis is already stopped. Tap to resume."*
...
tap should not pause Jarvis.

**Accessibility.** Stop announces *"Jarvis stopped"* through
`TYPE_ANNOUNCEMENT`. The confirm sheet has dialog semantics. Voice
trigger *"Jarvis, stop everything"* is honored in every voice mode.
...
- **Secret-shaped content.** Gateway refuses with classification
  *secret-shaped*; inline red note: *"That looks like a secret.
  Jarvis won't store secrets. Edit out the credential and try
...
*"Memory edit queued — send when Jarvis is back"*. The local
...
confirm (not direct stop) so a misfire does not pause Jarvis. Long
...
1. **Persistent banner** appears across the app: *"Mock mode is on
   — Jarvis is not connected to a real gateway."*
...
5. Toggling mock off opens a confirm sheet: *"Switching to live —
   Jarvis will start using your real gateway. Continue?"* On
...
Jarvis Prime running fully on the phone, with the app talking to
`http://127.0.0.1:8080`.
...
6. Owner uses the app exactly as in any other live mode. Tasks,
   ...
   Council, JARVIS Prime runtime are all on this phone.
...
Jarvis on this phone"* and *"Stop Jarvis on this phone"* for
...
1. Each permission is requested **with context first**: the app
   ...
   - **Foreground service.** *Gained: Jarvis can keep the gateway
...
Jarvis Prime silently fails. Every feature that depends on a
...
**Trigger.** Jarvis is in *Stopped* state. Owner is on **Control**
(or taps the persistent *"Jarvis is stopped. Tap to resume."*
...
2. Resume confirm sheet rises with:
   ...
   - **Approvals pending at stop time** that need re-review (with
     direct links).
   - The confirm phrase: *"Resume Jarvis"* (tap) or *"Resume
     Jarvis"* (voice).
...
- **Self-check fails** (gateway unreachable). Sheet shows *"Jarvis
...
the *Resume* button is dimmed with *"Resume needs Jarvis online —
