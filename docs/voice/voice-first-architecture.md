...
Phase 19 — voice-to-text job intake and the contract between Hermes
and the mobile cockpit / Termux gateway.

> **Audience:** anyone wiring a voice surface (Android cockpit, Termux
> gateway, CLI, an external app talking to the gateway) to Hermes.
> Implementers of the *audio* side (sounddevice, faster-whisper,
> Groq, etc.) — see `hermes_cli/voice.py`.

## 1. Goals

- Let a user create a Hermes job by talking — including hands-free
  while driving — without surprising them.
- Keep raw audio off Hermes' disk unless the user explicitly opts in.
...
- Hermes does **not** ship its own audio capture stack. The platform
  ...
  ever consumes text.
- Hermes does **not** continuously upload raw audio. If a user enables
