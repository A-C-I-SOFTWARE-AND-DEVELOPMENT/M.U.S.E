...
`driving_capture` is the only Hermes voice mode designed for use
...
- Hermes ships `driving_capture` as **opt-in**. A factory build
...
- This policy applies to voice intake only. It does not authorise
  Hermes to *do* anything risky on the user's behalf — see the
  publish gate in §5.
...
- Hermes restarts (the mode is not sticky across process restarts
...
- **No "I've been silent for 30 minutes, let me check in" prompts.**
  Driving mode is reactive: the user wakes Hermes, not the other way
