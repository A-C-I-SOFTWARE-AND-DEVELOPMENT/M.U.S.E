...
muse is **voice-first** in the sense that you can drive every
...
Three capture surfaces. All three end up as a normal muse turn on
the same backend.
...
Send a voice memo to your muse bot DM exactly as you'd send one
...
The mobile cockpit supports an **optional** wake word ("Hey muse"
...
muse will choose a short-form summary path automatically when TTS
runs into the cap.
...
- *"Hey muse start driving mode."*
- Cockpit → **Settings → Driving mode → On.**
- *"Hey muse stop driving mode."*
...
| Phrase | Effect |
|--------|--------|
| *"Hey muse start driving mode."* | Activates driving mode. |
| *"Hey muse stop driving mode."* | Deactivates driving mode. |
| *"muse cancel."* | Cancels the current capture before submission. |
| *"muse cancel job."* | Cancels the most recent job. |
| *"muse what's the status?"* | Reads back `muse orchestrator status` summary. |
| *"muse repeat that."* | Re-plays the last TTS reply. |
| *"muse save that as a memory."* | Stores the previous turn as a memory entry. |
| *"muse new conversation."* | Starts a fresh session (same as `/new` in chat). |
| *"muse approve."* | Approves the topmost queued approval (only when not driving). |
