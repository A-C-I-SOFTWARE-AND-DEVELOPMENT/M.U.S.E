...
|------------------------|--------------------------|------------------|-----------|---------------|------|---------------------------------|
| **Tailscale**          | Identity (SSO + ACL)     | Tailnet only     | WireGuard | DERP relay    | Free for personal use | Default. muse mobile + Windows desktop. |
...
`<workspace_root>` in the muse endpoint config.
...
* **Tailscale + SMB.** Share a Windows folder over SMB, set the
  ...
  only, and mount it from the muse host. Higher throughput than
...
muse host SSH into the VPS to read it.
...
```bash
# On the muse side, mounting from the VPS
...
Windows side, tunneled out, mounted from the muse host via
...
The original muse Local Orchestrator flow ([see
...
The muse config does not care which transport you picked — it only
