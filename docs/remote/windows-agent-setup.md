...
official `claude` CLI on Hermes' behalf, and writes artifacts back.
...
```
        ...
        └──────────────────────┬─────────────────────┘
         ...
         4. Refuse if device_id (Hermes side) is not approved.
        ...
        12. Move on to next job. Never delete files Hermes wrote.
```
...
|--------------------------|-----------|-----------|-------|
| `manifest.json`          | Hermes → worker | yes | Read-only on the worker side. Do not modify. |
| `prompt.md`              | Hermes → worker | yes | The text to drive `claude` with. |
| `cancel.json`            | Hermes → worker | optional | If it appears mid-run, abort and set `state: canceled`. |
| `status.json`            | both sides | yes | Worker overwrites with the latest state at least every `manifest.created_at + N`. |
| `output.md`              | worker → Hermes | yes | Narrative summary; usually `claude`'s stdout. |
| `patch.diff`             | worker → Hermes | when code changes proposed | Unified diff of the working tree. Omit when nothing changed. |
| `changed-files.txt`      | worker → Hermes | yes | `git diff --name-only`. Empty file is OK. |
| `validation-output.txt`  | worker → Hermes | yes | Tests / build output. Empty when no validation command was supplied. |

Anything outside this list (caches, scratch state) belongs **outside**
the shared directory so it does not get synced to Hermes.
...
3. **`allow_remote_execute` gate.** If the field is `false`, do not
   ...
   move on. Hermes may rewrite the manifest later with the field
...
this repo on purpose (the Hermes process and the Windows daemon
...
* expose Windows-side health to Hermes (e.g. by writing a heartbeat
  file at `<workspace_root>/worker-heartbeat.json`).

## Verifying the setup

1. On the Hermes side, write a tiny test job:
   ```python
...
3. Watch it transition to `state: completed` and confirm
   `output.md` shows up on the Hermes side.
...
|---------|--------------|-----|
| `state: awaiting_approval` indefinitely | Endpoint config has `allow_remote_execute: false`, or the daemon's manifest read finds `allow_remote_execute: false`. | Call `bridge.approve(job_id)` from Hermes — the bridge rewrites the manifest with the flag flipped. |
...
| Tunnel dropped, Hermes can't see status | Expected — the bridge surfaces `state: unknown` until the tunnel is back. | No action needed unless the tunnel stays down longer than
...
- [ ] Audit log on the Hermes side ships to a long-term sink
