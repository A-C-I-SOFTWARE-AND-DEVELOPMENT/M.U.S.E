...
This guide is for running muse against a **Windows** host that
runs Claude Code (or a Claude Code session running on Windows that
you want to drive from a muse backend elsewhere).

> **Heads-up:** Native Windows muse is **early beta**. WSL2 is the
...
The Claude Code Windows bridge is a small adapter that lets a muse
worker drive a **Claude Code** session running on Windows as if it
were a muse worker environment. It exists because:

- Some users have Claude Code subscriptions and want to point muse
...
- Some shops keep Windows for compliance reasons and want muse
...
- with **muse' validation gates, ledger, and approvals** in front
  of it.
...
The bridge supports two shapes depending on where the muse backend
lives.

### Shape A: muse backend on Linux, Claude Code on Windows
...
This is the recommended shape. muse' backend runs where it's
happiest (Linux), and only the worker environment lives on Windows.

### Shape B: muse backend on Windows native

```
[ Your phone / laptop / CLI ]
            │
            ▼
  [ muse backend (Windows native) ]
    ── ledger ── gates ── kanban ──
            │
            ▼  (in-process)
  [ Claude Code (Windows) ]
...
Works, but pulls muse' own runtime onto Windows. Use this only if
...
### 4. Configure a muse profile that uses the bridge

In `~/.hermes/config.yaml` (on whichever host runs the muse
backend):
...
profiles:
  windows-engineer:
    ...
    # selection. muse routes the *phase* here; Claude Code handles
    # generation.
```
...
5. Output streams back to the muse ledger and the user surface.
...
| Step | Runs on | Why |
|------|---------|-----|
| Orchestrator skill (decomposes the goal) | muse backend | The orchestrator profile lives in muse |
| Kanban dispatcher | muse backend | Source of truth for phase state. |
| Validation gates (schema, policy, judge) | muse backend | Gates aren't optional — they apply to bridge workers too. |
| Decision ledger | muse backend | Every bridge invocation logs `kind=spawn`, `kind=tool_call`, etc. |
...
- **Input.** The muse phase's `input.md` is fed in as the prompt.
...
- **Validation.** The muse side runs the schema gate (did Claude
...
- The muse backend trusts the Windows host enough to ship prompts,
  diffs, and (sometimes) source code to it.
- The Windows host trusts the muse backend enough to take its
  instructions.
...
- **No API keys cross the bridge.** muse does **not** send your
...
# On the host running the muse backend
muse plugin enable claude-code-bridge
