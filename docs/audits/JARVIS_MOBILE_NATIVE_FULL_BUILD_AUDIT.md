# JARVIS Prime — Mobile-Native Full-Build Audit
...
The goal is to finish JARVIS Prime as a mobile-first/mobile-native Android
...
- The cockpit API (`gateway/cockpit/`) already serves a **broad, real** surface
  (~25 routes) backed by the actual JARVIS Prime runtime, `JobQueue`,
...
| JARVIS Prime runtime | `hermes_cli/jarvis_prime/` | `__main__.py` (perceive/classify/gate/handle/stop/forget/remember/recollect/tick), `memory.py`, `research_vault.py`, `avatar.py`. |
