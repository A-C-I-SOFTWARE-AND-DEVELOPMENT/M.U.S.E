...
Wave 0 foundation. Stdlib-only, Termux-friendly, no heavy Hermes imports
at module import time. This module captures the structured intent of a
JARVIS unit of work so downstream layers (router, gates, owner auth,
verification) can reason about it uniformly without re-deriving fields
from free-form prose.

Design constraints:

* No third-party dependencies (no pydantic, no attrs).
* No network, filesystem, or subprocess calls at import or construction.
* Owner-gated actions are recorded as data only; this module never
  executes them.
* Confidence is clamped to ``[0.0, 1.0]``; invalid risk classes are
  reported through :meth:`WorkPacket.validate`, not raised, so callers
  can surface findings to the owner instead of crashing.
"""
...
def _coerce_str_list(value: Any) -> list[str]:
    """Best-effort coerce an iterable of strings into a list of strings.

    Strings are wrapped in a single-element list (not iterated character
    by character). ``None`` becomes an empty list. Anything else is
    passed through ``list()`` with ``str()`` applied to each element so
    downstream serialization stays JSON-clean.
    """
    if value is None:
        return []
...
def _clamp_confidence(value: Any) -> float:
    """Clamp a confidence-shaped value into ``[0.0, 1.0]``.

    Non-numeric input collapses to ``0.0`` rather than raising, because
    a partially-formed packet should still be inspectable; the
    :meth:`WorkPacket.validate` step is the right place to surface a
    finding about it.
    """
    try:
...
class WorkPacket:
    ...
    actions are stored verbatim and must be approved by the owner via
    the authorization phrase before any downstream executor acts on
    them.
    """
    ...
    def to_dict(self) -> dict[str, Any]:
        """Return a JSON-serializable dictionary of this packet."""
        return asdict(self)
    ...
    def from_dict(cls, data: dict[str, Any]) -> "WorkPacket":
        ...
        packets produced by newer versions without crashing.
        """
    ...
    def validate(self) -> list[WorkPacketValidationFinding]:
        ...
        This intentionally never raises. Callers (owner UI, gates, the
        router) can surface the findings to the user and decide whether
        to proceed.
        """
    ...
    def is_valid(self) -> bool:
        """Return ``True`` when :meth:`validate` reports no findings."""
