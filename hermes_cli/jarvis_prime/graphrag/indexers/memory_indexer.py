...
Each active :class:`MemoryNode` becomes a DECISION node (the cognition plane's
durable, owner-facing record). Its provenance pointers become SOURCE nodes with
CITES edges; ``supersedes`` becomes SUPERSEDES edges; and contradiction reports
become CONTRADICTS edges. This makes "what decided this / what contradicts it"
walkable from the graph.
...
def index_memory(
    ...
    for mem in store.nodes.values():
        ...
        # Provenance pointers -> SOURCE nodes, DECISION cites SOURCE.
        for s in mem.sources:
            ...
            src_node = graph.add_node(
                ...
                uri,
                title=uri,
                attrs={"trust": getattr(getattr(s, "trust", None), "value", "")},
                sources=[{"uri": uri, "kind": "memory_source"}],
            )
