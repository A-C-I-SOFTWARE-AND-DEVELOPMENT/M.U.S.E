...
* :func:`coding_query` — retrieve the code + tests + docs + prior decisions a
...
_CODE_NODE_TYPES = (NodeType.FILE, NodeType.FUNCTION, NodeType.CLASS, NodeType.SCREEN)
...
def coding_query(graph: KnowledgeGraph, question: str, *, limit: int = 6) -> GraphAnswer:
    ...
    tests, the docs/sources that cite them, and any prior decisions touching
    them — so the worker reuses existing implementations.
    """

    seeds = seed_nodes(graph, question, limit=limit, node_types=_CODE_NODE_TYPES)
    keep: set[str] = {n.id for n in seeds}
    for n in seeds:
        # Tests + verifications (incoming TESTS/VERIFIES), docs/sources that
        # cite this node (incoming CITES), and prior decisions/tasks.
        keep.update(
            graph.neighbors(
                n.id,
                depth=1,
    ...
    def _rank(n: Node) -> tuple:
        type_bonus = {
            ...
            NodeType.SCREEN: 0, NodeType.DECISION: -1, NodeType.SOURCE: -2,
            NodeType.DOCUMENT: -1,
        }.get(n.type, -3)
...
# ---------------------------------------------------------------------------
# Related-items (powers the cockpit "related files/sources/decisions" panel)
# ---------------------------------------------------------------------------
...
_RELATED_BUCKET = {
    ...
    NodeType.TASK: "decision",
    NodeType.WORKER: "decision",
    NodeType.MODEL: "decision",
}
...
def find_entity_node(
    graph: KnowledgeGraph, *, node: Optional[str] = None, key: Optional[str] = None
) -> Optional[str]:
    """Resolve a node id from an explicit id, or a key matched across types.

    Used by the cockpit ``/graph/related`` endpoint to turn a ``job_id`` /
    ``memory_id`` / ``node`` query param into a graph node id.
    """

    if node and node in graph.nodes:
        return node
    if key:
        # Try common key shapes used by the indexers.
        candidates = [
            key,
            f"memory:{key}",
            f"ledger:{key}",
