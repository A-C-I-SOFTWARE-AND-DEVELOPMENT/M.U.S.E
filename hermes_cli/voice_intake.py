...
(``hermes_cli.voice``) — recording, transcription, TTS. This module
...
5. ``finalize(intake)`` — for ``approved`` intakes, hand the prompt
   to ``hermes_cli.orchestrator.submit_job`` and return the new job
   ID; for ``cancelled`` / ``expired`` intakes, return ``None``.

The pipeline never touches a microphone or a TTS provider directly —
``hermes_cli.voice`` already does that, and keeping intake decoupled
...
def _hermes_home() -> Path:
    ...
    Same lookup ``hermes_cli.orchestrator`` uses, so voice artefacts
...
def finalize(
    ...
    *submitter* is the function that will create the orchestrator
    job. The default uses ``hermes_cli.orchestrator.submit_job`` —
...
def _default_submitter(prompt: str) -> Any:
    from hermes_cli.orchestrator import submit_job
