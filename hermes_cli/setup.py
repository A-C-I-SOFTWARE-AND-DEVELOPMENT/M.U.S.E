...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in muse (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│          ◉ muse Setup — Nous Portal (one-shot)          │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice(
            "How would you like to set up muse?",
            [
