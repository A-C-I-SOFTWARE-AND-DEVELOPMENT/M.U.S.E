"""
Interactive setup wizard for muse
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ muse Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    ...
    if detected_claude or detected_codex:
        ...
        print_info("Configure muse using environment variables or config commands:")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        ...
        try:
            ...
            print_info(
                "muse can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one muse can auto-use for vision,")
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where muse runs shell commands and code.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where muse delivers cron job results,")
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="muse",
            bot_description="Your muse agent on Slack",
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where muse delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where muse delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects muse to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with muse from anywhere.")
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure muse to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point muse at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point muse at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point muse at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point muse at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to muse equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing muse config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in muse", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ muse's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("muse can preview what would be imported before making any changes.")
    print()
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in muse (use muse claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ◉ muse Setup — {label:<32s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│                ◉ muse Setup Wizard                 │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your muse installation.           │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have muse configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'hermes setup model|terminal|")
        print_info("     gateway|tools|telemetry|agent|trio', or fill only missing items")
        print_info("     with --quick.")
        ...
        setup_mode = prompt_choice("How would you like to set up muse", [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect muse to messaging apps to chat from anywhere.")
