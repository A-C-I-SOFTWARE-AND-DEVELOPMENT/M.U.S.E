...
The lease kernel (:mod:`hermes_cli.worker_lease`) decides when a lease is
``EXPIRED`` and whether it ``can_retry`` (idempotent only); the store
(:mod:`hermes_cli.worker_lease_store`) persists leases + a host registry. What
...
:class:`hermes_cli.runtime_adapter.RuntimeAdapter`. It never mutates the store
...
from hermes_cli.worker_lease import LeaseStatus, WorkerLease, can_retry
