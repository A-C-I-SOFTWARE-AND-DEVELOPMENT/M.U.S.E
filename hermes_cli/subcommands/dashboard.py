...
def _add_server_runtime_args(parser) -> None:
    ...
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running muse web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running muse web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the muse backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the muse backend server — the JSON-RPC/WebSocket gateway the "
