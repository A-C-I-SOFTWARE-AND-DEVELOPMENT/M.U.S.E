...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/muse.app`` or ``~/Applications/muse.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\muse`` (NSIS per-user)
...
the app name ("muse"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/muse``
  - Windows: ``%APPDATA%\\muse``
  - Linux:   ``$XDG_CONFIG_HOME/muse`` (default ``~/.config/muse``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "muse"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "muse"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "muse" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "muse")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "muse.desktop",
        ]
    return paths
