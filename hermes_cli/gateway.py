...
SERVICE_DESCRIPTION = "M.U.S.E. Gateway - Messaging Platform Integration"
...
def print_legacy_unit_warning() -> None:
    ...
    if not legacy:
        return
    print_warning("Legacy muse gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy muse gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy muse gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def install_linux_gateway_from_setup(
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so muse can't create it from this user session.")
            if run_as_user:
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current muse install")
    return True
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current muse install")
    return True
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current muse install")
    else:
        print("⚠ Service definition is stale relative to the current muse install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the muse gateway as root inside the official Docker image."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ✦ M.U.S.E. Gateway Starting...               │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and muse will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where muse delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your muse agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address muse will use (e.g., hermes@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. muse connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and muse will connect automatically over WebSocket",
        ],
        "vars": [
...
def _setup_weixin():
    ...
    print()
    print_info("  1. muse will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. muse will store the returned account_id/token in ~/.hermes/.env.")
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"M.U.S.E.\"")
...
def _ensure_via_s6_or_background() -> int:
    ...
    pre-registered at boot (see :mod:`hermes_cli.container_boot`), so all
    "establish" has to do is bring the slot up. On plain Docker (no s6)
    there is no supervisor, so fall back to a detached background run.
    """
    try:
        from hermes_cli.profiles import get_active_profile_name
    ...
    if detect_service_manager() == "s6":
        slot = f"gateway-{get_active_profile_name()}"
        try:
...
def gateway_ensure(verbose: int = 0) -> int:
...
def _gateway_command_inner(args):
    ...
    if subcmd == "ensure":
        rc = gateway_ensure(verbose=getattr(args, 'verbose', 0))
        if rc:
            sys.exit(rc)
        return
