...
def render_message(text: str, cols: int = 80) -> str | None:
    try:
        from agent.rich_output import format_response  # ty: ignore[unresolved-import]
    except ImportError:
        return None
...
def render_diff(text: str, cols: int = 80) -> str | None:
    try:
        from agent.rich_output import render_diff as _rd  # ty: ignore[unresolved-import]
    except ImportError:
        return None
...
def make_stream_renderer(cols: int = 80):
    try:
        from agent.rich_output import StreamingRenderer  # ty: ignore[unresolved-import]
    except ImportError:
        return None
