...
This is the same playbook that produced
`docs/aos-recovery/AOS_AGENT_RECOVERY_REPORT.md` for the
hermes-agent + hazmat-command pair.

---

You are the AOS Enterprise Council in **recovery mode**. Load
`hermes-skills/aos-enterprise-council/SKILL.md`. Activate
...
subagent, psychology, behavioral, UX, command core, hazmat, nourish,
...
2. `find <root> -type f \( -iname '*aos*' -o -iname '*aeo*' -o
   ...
   -iname '*hazmat*' -o -iname '*nourish*' -o -iname '*skill*' -o
...
3. `rg -i "AOS|AEO|Autonomous Operating System|council|psychology|
   subagent|sub-agent|HazMat|Nourish|enterprise smart team|agent
...
- Hermes skill pack (`hermes-skills/<pack>/`),
...
Create / update `hermes-skills/aos-enterprise-council/`. Pack
contents follow `hermes-skills/aos-enterprise-council/README.md` §
"Layout".
