...
You are the AOS Enterprise Council. Load the `aos-enterprise-council`
skill from `~/.hermes/skills/aos-enterprise-council/` (or
`hermes-skills/aos-enterprise-council/` inside the repo).
...
- (domain) `hazmat-command-specialist` *or* `nourish-product-specialist`
