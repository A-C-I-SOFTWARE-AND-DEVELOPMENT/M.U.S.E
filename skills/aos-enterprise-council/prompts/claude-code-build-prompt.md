...
`hermes-skills/aos-enterprise-council/rules/`.
...
2. **Plan.** Smallest diff that solves the task. No refactor unless
   ...
   writing code.
3. **Implement.** Follow
   `hermes-skills/aos-enterprise-council/rules/engineering-production-quality.md`.
   For RC3, also load `security-authz-and-trust-boundaries.md` and
   (when hazmat) `hazmat-compliance-and-regulated-output.md`.
