...
│                                                                      │
│   NodeServer (from `muse meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `muse meet` CLI |
...
| `cli.py` | `muse meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `muse meet node {run,list,approve,remove,status,ping}` |
...
muse meet install                                      # pip + Chromium
muse meet setup                                        # preflight
muse meet auth                                         # optional
muse meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
muse meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hermes/.env
muse meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
muse meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
muse meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
...
muse plugins enable google_meet
muse meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
muse meet node approve my-mac ws://<mac-ip>:18789 <token>
muse meet node ping my-mac
