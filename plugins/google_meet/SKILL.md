...
muse meet install                 # pip deps + Chromium (transcribe only)
muse meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
muse meet auth                    # optional; skips guest-lobby wait
muse meet setup                   # preflight checks
```

`muse meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
muse plugins enable google_meet
muse meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
muse meet node approve my-mac ws://<mac-ip>:18789 <token>
muse meet node ping my-mac                   # confirm reachable
```

Run `muse meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `muse meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `muse meet node approve`.
